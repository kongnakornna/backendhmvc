...
######## Cach Toools Start  ################################################
            ...
            #$cachetime=$this->_ci->config->item('cachetime8');
            ...
            $cachetime=60*60*60*24*365*100;
            ##Cach Toools Start######
            //cachefile 
            $input=@$this->_ci->input->post(); 
            if($input==null){ $input=@$this->_ci->input->get();}
            ...
            if($cachechklist!=null){    // IN CACHE
                 ...
                 # echo'<pre> 1 last_query=>';print_r($last_query);echo'</pre>';Die();
                 // if($deletekey==1){echo 'last_query-> '.$last_query;}    die(); 
                  ...
                  $datasave=$users;
                  $sql=null;
                  $data_result=$this->_ci->Cachtool_model->cachedbsetkey($sql,$datasave,$cachekey,$cachetime,$cachetype,$deletekey);
                  #echo'1 Form SQL <hr> <pre>   data_result =>';print_r($data_result);echo'</pre>'; Die();
...
######## Cach Toools Start  ################################################
            ...
            if($cachechklist!=null){    // IN CACHE
                  ...
                  //$query = $this->_ci->db->select("*")->from("_admin_menu")->where($array_menu)->order_by($orderby, 'ASC')->get();
                  $last_query=$this->_ci->db->last_query();
                #  echo'<pre> last_query=>';print_r($last_query);echo'</pre>';Die();
                 // if($deletekey==1){echo 'last_query-> '.$last_query;}    die(); 
                  ...
                  $sql=null;
                  $data_result=$this->_ci->Cachtool_model->cachedbsetkey($sql,$datasave,$cachekey,$cachetime,$cachetype,$deletekey);
                   #echo'2 Form SQL <hr> <pre>   data_result =>';print_r($data_result);echo'</pre>'; Die();
