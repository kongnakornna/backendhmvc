...
class Accessuser_library {
private $_ci;
function __construct(){
      $this->CI=& get_instance(); 
      $CI =& get_instance();
      $CI->load->database();
      $this->CI->load->library('session');  // เรียกใช้งาน session
      $this->CI->load->helper('cookie','session');
      $this->CI->load->helper('cookie');  // เรียกใช้งาน cookie
...
public function session_cookie2_get(){
          $this->CI->load->library('session');
...
public function cookie_set($cookie_name,$cookie_value,$time){
...
public function setDataCookie($cookie_name,$cookie_value,$time){
     ...
     $cookie=array(
               ...
               'value' => $cookie_value,                            
               'expire' => (time()+$time),     
               //'domain' => 'http://connexted.org/ireport', 
               'path' => "/",                                                                          
               'secure' =>false
          );
     $cookiert=$this->CI->input->set_cookie($cookie);
...
public function encryptCookie($value){
     $this->_CI=&get_instance();
     $this->_CI->config->load('encryptkey'); 
     $setoption=$this->_CI->config->item('setoption'); 
     if( $setoption===0){
          return $value;
     }else{ 
          ################################
...
public function cleantempsessionandcookie(){
     ...
     #####################################################################
          ...
          if($temp_cookie_user_id==null){}else{
...
public function cleansummonsessionandcookie(){
          ...
          $this->CI->load->library('Accessuser_library');
           ...
           $_COOKIE=$session_cookie_get['COOKIE'];
           $SESSION=$session_cookie_get['SESSION'];
           $user_id=@$_COOKIE['summon_user_id'];
           if($user_id==null){}else{
