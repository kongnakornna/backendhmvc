...
class Image_cache 
{
    ...
    function __construct()
    {
        ...
        $this->dest_path= $this->CI->config->item('root_base_path');
    }
    ...
    function image_cache()
    {
        ...
        if (( ! file_exists($this->dest_path . $this->new_image)) || filemtime($this->dest_path . $this->new_image) < filemtime($this->source_image) || $this->replace_original) 
        {
            if ( ! is_dir($this->dest_path . dirname($this->new_image)))
            {
                mkdir($this->dest_path . dirname($this->new_image), 0777, TRUE);
            }
        ...
        return static_url($this->new_image);
    }
    ...
    {
        $this->source_image = rtrim(urldecode($this->source_image), '/');
        // Alternative image if file was not found
        if ( ! file_exists($this->source_image) || $this->source_image == '')
        {
            $this->source_image = rtrim(urldecode($this->no_image_image), '/');
        }
    ...
    function _new_image()
    {
        ...
        $ext = $info['extension'] ? $info['extension'] : 'png';                                                                                                                          
        ...
        {
          $md5=md5($dirname . '/' . $filename . '.' . $ext);
            if ($this->crop)
            {
                $new_image = $image_cache . '/' . substr($md5, 0,16).'/'. substr($md5, 16,32). '/' . $filename . '-' . $this->width . 'x' . $this->height . 'c' . '.' . $ext;
            }
            else
            {
                $new_image = $image_cache . '/' . substr($md5, 0,16).'/'. substr($md5, 16,32). '/' . $filename . '-' . $this->width . 'x' . $this->height . '.' . $ext;
            }
        }
    ...
    {
        //The original sizes
        $original_size = getimagesize($this->source_image);
        ...
        $config['new_image'] = $this->dest_path.$this->new_image;
        ...
        if (($this->width != 0) && ($this->height != 0)) 
        {
            ...
            //Cropping
            $config = array();
            $config['source_image'] = $this->dest_path.$this->new_image;
            $config['maintain_ratio'] = FALSE;
            $config['new_image'] = $this->dest_path.$this->new_image;
