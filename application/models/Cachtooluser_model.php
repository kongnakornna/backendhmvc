...
class Cachtooluser_model extends CI_Model {
private $add;
private $edit;
private $delete;
private $update;
...
public function cache_list_file(){
               ...
               foreach ($in as $k => $v){
               if (!is_array($v)){?>
                    <option><?php   echo $path,$v  ?></option>  <?php 
                    }else{  print_dir($v,$path.$k.DIRECTORY_SEPARATOR);  }
                    }
               }
...
public function cachedbgetkey($sql,$cachekey,$cachetime,$cachetype,$deletekey){
     ############cache##################
          ...
          */
          ...
          /* echo '<br> cachetype=>'.$cachetype;  echo '<br> sql=>'.$sql;  Die(); */
          /* ############# 1cache DB Start##############  */  
          ...
          if($cachetype==2){
                    ...
                    $path=$this->config->item('cache_path_usertype');
...
/*
	...
	$deletekey=@$input['deletekey']; // 1=delete
	$this->load->model('Cachtooluser_model');
	$cachekey="key-district-".$lang;
	$cachetime=60*60*24*365;
	//cachefile 
	$cachetype=2;
	$sql=null;
	$cachechk=$this->Cachtooluser_model->cachedbgetkey($sql,$cachekey,$cachetime,$cachetype,$deletekey);
	...
	if($cachechklist!=null){
          ...
          $dataresult_all=array('dataresult'=>$dataresult,'cache_msg'=>$cache_msg,'cache_key'=>$cache_key,'cachetime'=>$cachetime);
               ...
               $dataresult_all=array('dataresult'=>$dataresult,'num'=>$num,'cache_msg'=>$cache_msg,'cache_key'=>$cachekey,'cachetime'=>$cachetime);
