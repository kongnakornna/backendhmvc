...
class User_menu_Model extends CI_Model{
    ...
    public function get_max_id(){
		...
		$this->db->select('max(user_menu_id) as max_id');
		$this->db->from('_user_menu');
		$query = $this->db->get();
    ...
    public function get_status($id){
    	...
    	$this->db->from('_user_menu');
    	$this->db->where('user_menu_id2', $id);
	...
	public function getMenu($id = 0, $parent = 0, $status = '', $curlang = null){
		...
		$language = $this->lang->language['lang'];
		$this->db->select('*, (SELECT COUNT(user_menu_id) FROM `sd_user_menu` as hm2 WHERE hm2.`status`=1 AND hm2.`lang` = "'.$language.'" AND hm2.`parent` = hm.user_menu_id2) AS count_sub');
		$this->db->from('_user_menu as hm');

		if($status != '') $this->db->where('status', $status);
		if($id > 0) $this->db->where('user_menu_id2', $id);
    ...
    function store($user_menu_id = 0, $data){

			//echo "(user_menu_id = $user_menu_id)";

			if($user_menu_id > 0){
					$this->db->where('user_menu_id', $user_menu_id);
					$this->db->update('_user_menu', $data);
					$report = array();
					...
					$this->db->insert('_user_menu', $data);
					//echo $this->db->last_query();
	...
	function status_new($user_menu_id, $enable = 1){

		$data['status'] = $enable;
		$this->db->where('user_menu_id2', $user_menu_id);
		$this->db->update('_user_menu', $data);
		//echo $this->db->last_query();
		$report = array();
		////$report['error'] = $this->db->_error_number();
	...
	function update_order($user_menu_id, $order = 1, $parent = 0){

		$data['order_by'] = $order;
		$this->db->where('user_menu_id2', intval($user_menu_id));
		$this->db->where('parent', $parent); 
		$this->db->update('_user_menu', $data);
		//Debug($this->db->last_query());
