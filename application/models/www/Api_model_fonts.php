...
class Api_model_fonts extends CI_Model {
	...
	function urlna($urlcerentrl,$lang){
		...
		$tbl_user_menu = $this->db->dbprefix('_user_menu');
		$query = $this->db->get($tbl_user_menu);
         $num_rows=$query->num_rows();
		 //echo $this->db->last_query();die();
         if($num_rows>0){
              foreach ($query->result_array() as $row){
					$user['user_menu_id'] = $row['user_menu_id'];
					$user['user_menu_id2']=$row['user_menu_id2'];
                    $user['title']=$row['title'];
                    $user['url']=$row['url'];
                    $user['parent']=$row['parent'];
                    $user['user_menu_alt']=$row['user_menu_alt'];
                    $user['option']=$row['option'];
                    $user['icon']=$row['icon'];
                    $user['params']=$row['params'];
    ...
    public function user_menu($type){	
			...
			$loadfile = "usertype".$type.".json";
			$list_data = $title = '';
			//if($type == 1) $loadfile = "superuser.json";
			$user_menu = LoadJSON($loadfile);
			$subuser_menu = $user_menu;			
			  //Debug($subuser_menu);
			if($user_menu){
				foreach($user_menu as $arr => $list_mainmenu){
						$sub_mainparent = '';
						$havesub = 0;	
                       $user_menu_id=$list_mainmenu['user_menu_id'];				
						if($list_mainmenu){
							foreach($list_mainmenu as $field => $title){
									//echo "$field => $title<br>";
									if($field == "user_type_id") $user_type_id = $title;
									if($field == "user_menu_id") $user_menu_id2 = $title;
...
$title = ($lang == 'th') ? $title_th : $title_en;
       /*************Sub menu Active****************/
                            ...
                            $urlna = $this->Api_model_fonts->urlna($urlna,$lang); 
                            $num_rowsna=(int)$urlna['num_rows'];
                            //Debug($urlna); Die();
        if($num_rowsna>0){
        $user_menu_id2na=$urlna['user_menu_id2'];
        $paramsna=$urlna['parent'];
        if(($user_menu_id2 == $paramsna) && ((strtolower($this->uri->segment(1)) == $urlcerentrl)	)){ 
            ...
            //////////////
            if(($user_menu_id2 == 1) && ((strtolower($this->uri->segment(1)) == $urlcerentrl))){
                                        $curactive = 'class="active open"';
                                        $submenu= 'class="class="sub-menu"';
									}else{
										$curactive = '';
       ...
       /*************Sub menu Active****************/
							...
							if($subuser_menu){
								foreach($subuser_menu as $subarr => $list_mainmenu2){
									//Debug($list_mainmenu2);
										foreach($list_mainmenu2 as $subfield => $subtitle){
											//echo "$subfield => $subtitle<br>";	
											if($subfield == "user_type_id") $sub_user_type_id = $subtitle;
											if($subfield == "user_menu_id") $sub_user_menu_id = $subtitle;
											...
											if($subfield == "parent") $sub_mainparent = $subtitle;														//echo "($sub_mainparent == $user_type_id)<br>";																												
								            }
													
													if($sub_mainparent == $user_menu_id2){
    ...
    public function notification_birthday(){
			...
			$this->db->select('dp.*');
			$this->db->from('_user_profile as dp');
    ...
    public function user($language = 'th'){
        ...
        //$sql = "_user_profile.*, _user_type.user_type_id_map, _user_type.user_type_name";
        $sql = "dp.*, dt.user_type_name, bt.belong_to";
       
        $this->db->select($sql);
		$this->db->from('_user_profile as dp');
		//$this->db->join('_user_type', '_user_profile.user_type_id = _user_type.user_type_id_map AND `'.$prefix.'_user_type`.lang = "'.$language.'" ', 'left');
		$this->db->join('_user_type as dt', '(dp.user_type_id = dt.user_type_id_map AND dt.lang="'.$language.'"',  'left');
		...
		//echo "get_tag_pair(".$rs[0]->user_profile_id.", 5)";
        ...
        if($total_all > 0){
            $i=0;
            foreach($rs as $key => $row){
                $item[$i]['user_profile_id'] = $row->user_profile_id;
                $item[$i]['user_type_id'] = $row->user_type_id;
                ...
                $item[$i]['user_type_name'] = $row->user_type_name;
                $item[$i]['belong_to'] = $row->belong_to;

				$sel_tags = $this->tags_model->get_tag_pair($rs[$i]->user_profile_id, 5);
    ...
    public function news($language, $id = null){
		...
		if($id > 0){
		        ...
		        $sql = "_news.news_id2 as news_id, _news.title, _news.description, _news.start_date, _news.expire_date, _category.category_name, _subcategory.subcategory_name, _user_profile.first_name, _user_profile.last_name, _user_profile.nick_name, _user_profile.avatar";
		}
		...
		if(!$id) $this->db->join('_user_profile', '_news.user_id = _user_profile.user_profile_id', 'left');
    ...
    public function column($language){
        ...
        $sql = "_column.*, _category.category_name, _subcategory.subcategory_name, _user_profile.first_name, _user_profile.last_name, _user_profile.nick_name, _user_profile.avatar";
		...
		$this->db->join('_user_profile', '_column.user_id = _user_profile.user_profile_id', 'left');
    ...
    public function gallery($language){
        ...
        //$sql = "_gallery.*, _category.category_name, _subcategory.subcategory_name, _user_profile.first_name, _user_profile.last_name, _user_profile.nick_name, _user_profile.avatar";
        $sql = "_gallery.*, _gallery_type.gallery_type_name , _user_profile.first_name, _user_profile.last_name, _user_profile.nick_name, _user_profile.avatar";
		...
		$this->db->join('_user_profile', '_gallery.user_id = _user_profile.user_profile_id', 'left');
