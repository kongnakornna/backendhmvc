...
where am.status=1 ".$criticalCriteria."
	...
	$query = $DBSelect->query($sql, $arrWhere);
		...
		if (!$arrResult = $this->tppymemcached->get($cache_key)) {
			...
			$this->tppymemcached->set($cache_key, $arrResult, 3);
		}
		*/
		return $arrResult;
	}
...
*/
	...
	public function get_cmsblogCountContent($arrFilter = array()) 
	{
		...
		if (!$r = $this->tppymemcached->get($cache_key)) {
			...
			$this->tppymemcached->set($cache_key, $r,6);
		}
		*/
			$query = $DBSelect->query($sql);
			if($query){
	...
	*/
	...
	function get_blogger_status_by_blogger($user_id=0, $category_id_in='', $zone_id=1){
  ...
  function get_content_id_count($idx){
  ...
  function get_webboardpost($wb_post_id){ 
      $sql_1 = "SELECT webboard_room.wb_room_name,webboard_category.wb_room_id,webboard_category.wb_category_name,webboard_category.wb_category_parent_id,webboard_post.* FROM webboard_post  INNER JOIN webboard_category ON webboard_post.wb_category_id = webboard_category.wb_category_id INNER JOIN webboard_room ON webboard_category.wb_room_id = webboard_room.wb_room_id WHERE webboard_post.wb_post_id=$wb_post_id";
	  $query_1=$this->db->query($sql_1);
      $results_webboard_post=$query_1->result_array();
      
      $sql_2 = "SELECT * FROM webboard_reply WHERE wb_post_id=$wb_post_id";
	  $query_2=$this->db->query($sql_2);
      $results_webboard_reply=$query_2->result_array();
      
      $sql_3 = "SELECT * FROM webboard_log WHERE wb_id=$wb_post_id";
	  $query_3=$this->db->query($sql_3);
      $results_webboard_log=$query_3->result_array();
      
      $array_result=array('webboard_post'=>$results_webboard_post,
      					  'webboard_reply'=>$results_webboard_reply,
      					  'webboard_log'=>$results_webboard_log,
      					 );
      
    return $array_result;
  }
  function get_webboardpost_status_data($wb_post_id){ 
        $sql_2 = "SELECT wb_status FROM webboard_post WHERE wb_post_id=$wb_post_id";
	    $query_2=$this->db->query($sql_2);
        $result_data=$query_2->result_array();
         $result_data=$result_data['0'];
	    return $result_data;
  
  }
  
  function get_webboardpost_status($wb_post_id,$status){ 
        $data=array('wb_status'=>$status);
	    $this->db->where('wb_post_id',$wb_post_id);
	    $result_data=$this->db->update('webboard_post',$data); 
	    return $result_data;
  
  }
  function get_webboard_reply_status($wb_post_id,$status){ 
        $data=array('reply_status'=>$status);
	    $this->db->where('wb_post_id',$wb_post_id);
	    $result_data=$this->db->update('webboard_reply',$data); 
	    return $result_data;
  
  }
  function get_webboard_reply_status_id($wb_reply_id,$status){ 
        $data=array('reply_status'=>$status);
	    $this->db->where('wb_reply_id',$wb_reply_id);
	    $result_data=$this->db->update('webboard_reply',$data); 
	    return $result_data;
  
  }
    function get_webboardreply_status_data($wb_reply_id){ 
        $sql_2 = "SELECT reply_status FROM webboard_reply WHERE wb_reply_id=$wb_reply_id";
	    $query_2=$this->db->query($sql_2);
        $result_data=$query_2->result_array();
         $result_data=$result_data['0'];
	    return $result_data;
