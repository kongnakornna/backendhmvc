<?php
class webboardapi_model extends CI_Model {
	...
	public function getCountTopic($categoryid){
		$cache_key = "cache_getCountTopic+$categoryid";
...
//------------------
	...
	public function getTopic($room = null, $categoryid = null , $topicSearch = null , $order = null, $limit, $offset = null , $topicid = null, $arrFilter = array()) {
        // if(empty($room))
            // return false;
	...
	///////////////
	public function reply_count($postid){ 
		$DBSelect = $this->load->database('select', TRUE);
		$selTotalReply = "select count(*) as num_reply from webboard_reply where wb_post_id = '".$postid."' and reply_status = '1' ";
		$q_total_reply = mysql_query($selTotalReply, $DBSelect);
		$row_total_reply = mysql_fetch_assoc( $q_total_reply );
		$total_page = $row_total_reply['num_reply'];
		if( !$total_page )
			$total_page = 1;
		return $total_page;
	}
	...
	public function isFavorite($postid){ 
		$data=false; // true กับ false 
		return $data;
	}
	public function islike($topicid='',$user_id=''){ 
		$data=false; // true กับ false 
		return $data;
	}	
    public function deletewebborrd($topicid){ 
		$data=false; // true กับ false 
		return $data;
	}
}	