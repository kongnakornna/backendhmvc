...
######################TEST###################
    ...
    public function fileexists($fullpath,$file){
 		  if(file_exists($fullpath.$file)){
		  	      $status=1;
				  $mesage='Have file '.$file.' !';
				  $pathimage=$fullpath.$file;
				  $data=array(
	                'status'=>$status,
	                'mesage'=>$mesage,
	                'pathimage'=>$pathimage,
	...
	*/	
    ...
    public function get_resize_image($path, $thumbnail, $id, $width, $height){

		   $static_path_media=$this->config->item('static_path');
		   if($path<>''){
		   	 $path_file=$path;
		   }else{
           	$path_file=$static_path_media."cmsblog/".($id%4000)."/$id/";
		   }
           // $search=end(explode("/", $thumbnail));
           // list($first) = explode(".", $search);
           // $end = end(explode(".", $search));
           // $search=$first.'_thumb_file_'.$width.'x'.$height.'.'.$end;
           $search='thumb_file.jpg'; // คือ คำที่ต้องการลบ
           $replace1='thumb_file_'.$width.'x'.$height.'.jpg'; //คือ คำที่ต้องการใส่แทน
           $fullpath=$path_file;
           $string=$thumbnail;
 		   $thumbnail_resize1=str_replace($search,$replace1,$string);
		     $fullpath_image1=$this->fileexists($path_file,$replace1);
		     #echo '<pre> $fullpath_image1=>'; print_r($fullpath_image1); echo '</pre>';
 		     $fullpath_status1=$fullpath_image1['status'];
 		     $thumbnail1=$fullpath_image1['pathimage'];
 		     if(file_exists($fullpath_image1['pathimage'])){
 		     	$result = $thumbnail_resize1;
 		     }else{
 		     	if(file_exists($path_file.$search)){
		 		     	// echo '<pre> $thumbnail1=>'; print_r($path_file.$search); echo '</pre>';die;
		 		        $replace1='thumb_file_'.$width.'x'.$height; //คือ คำที่ต้องการใส่แทน
		 		     	$thumbnail_resize1=Null;
	     			    if($path<>''){
	     			    	$fullpath=$static_path_media.$path;
	     			    	$dir=$id.'/';
	     			    }else{
	     	           	 	$fullpath=$static_path_media."cmsblog/".($id%4000).'/'.$id.'/';
	     			    }
					 	$file=$search;
					 	$filenew_name=$replace1;
					 	$resizeimage1=$this->resizeimage($fullpath,$dir,$file,$filenew_name,$width,$height);
					 	//echo '<pre> $resizeimage1=>'; print_r($resizeimage1); echo '</pre>';die;
					 	$result = $thumbnail;
					 	// $result = $thumbnail_resize1;
 		     	}else{
 		     		$result = $thumbnail;
    ...
    public function checkfileresize($urlpath='',$fullpath='',$dir='',$file,$filenew_name,$width='',$height=''){
 	      $thumbnail=$file;
 	      if($fullpath==Null){
 	       $fullpath='/data/product/trueplookpanya/www/static/';
 	     }
 	           $path_file=$fullpath;  //  path รูป
 	           if($file==Null){
 	        // $search='thumb_file.jpg'; // คือ คำที่ต้องการลบ
 	         $search=$file; // คือ คำที่ต้องการลบ
 	     }
 	           if($filenew_name==Null){
 	       $filenew_name='thumb_file_792x446.jpg'; //คือ คำที่ต้องการใส่แทน
 	       $replace1=$filenew_name;
 	     }else{
 	      $replace1=$filenew_name;
 	     }
 	           $fullpath=$path_file;
 	           $string=$thumbnail; // รูปที่ส่งมาจากระบบ
 	      $thumbnail_resize1=str_replace($search,$replace1,$string);
 	       $fullpath_image1=$this->fileexists($path_file,$replace1); // ตรวจสอบว่ามีรูปหรือยัง
 	       #echo '<pre> $fullpath_image1=>'; print_r($fullpath_image1); echo '</pre>';
 	        $fullpath_status1=$fullpath_image1['status'];
 	        if($fullpath_status1==0){
 	         $thumbnail1=$fullpath_image1['pathimage']; 
 	         //echo '<pre> $thumbnail1=>'; print_r($thumbnail1); echo '</pre>';
 	         if($thumbnail1==Null){ 
 	            $replace=$replace1; //คือ คำที่ต้องการใส่แทน
 	          $thumbnail_resize1=Null; 
 	          $fullpath=$path_file;    //  path รูป
 	      $dir=$dir;   //  path รูป
 	      $file=$search;
 	      $filenew_name=$replace;
 	      if($width==Null){
 	      $width='792';
 	     }if($height==Null){
 	      $height='446';
 	      ...
 	      $resizeimage1=$this->resizeimage($fullpath,$dir,$file,$filenew_name,$width,$height);
 	      //echo '<pre> $resizeimage1=>'; print_r($resizeimage1); echo '</pre>';
 	      $status=$resizeimage1['status_file'];
 	      $mesage=$resizeimage1['message'];
 	      $image=$urlpath.$filenew_name;
 	         }
 	          $data = array(
 	                 'status' => $status,
 	                 'mesage' => $mesage,
 	     ...
 	     return $data;
 	        ###################
 	         ...
 	         $image=$urlpath.$filenew_name;
 	     $fullpath_image2=$this->fileexists($path_file,$filenew_name);
 	         $fullpath_status2=$fullpath_image2['status'];
 	         $thumbnail2=$fullpath_image2['pathimage'];
 	          $data = array(
 	                 'status' => $status,
 	                 'mesage' => $mesage,
 	                 'image' => $image,
    ...
    public function cutstring($str, $len) { 
        ...
        if ($len == 0) { 
         ...
         return (mb_strlen($str) <= $len || $len < 3) ? $str : mb_substr($str, 0, $len - 2, $unicode)."..."; 
        } 
    }
    ///Member
    ...
    public function add_Comment($data = array()) {
        if (!empty($data)) {
            $this->db->insert('webboard_reply', $data);
             $post_id=$data['wb_post_id'];
             $cache_key = "AdmissionWebboard_Detail_".$post_id; //$this->input->post('post_id');
	...
	public function switch_Comment($data = array(),$reply_id = 0) {
        if (!empty($reply_id)) {
            $this->db->where('wr.wb_reply_id', $reply_id);
            $this->db->update('webboard_reply wr', $data);
            return ($this->db->affected_rows() != 1) ? false : true;
        } else {
            return FALSE;
        }
    }
	public function getWebboard_id($post_id = 0) {
        if (!empty($post_id)) {
            
			$cache_key = "Webboard_Post_am_".$post_id;
			if (!$rsWB = $this->tppymemcached->get($cache_key)) {
			
				$this->db->select('wp.*')
						->from('webboard_post wp')
						->where('wp.wb_post_id', $post_id);
				$query = $this->db->get();
				if ($query->num_rows() > 0) {
					$rsWb = $query->result_array();
				} else {
					$rsWB = false;
				}
				$this->tppymemcached->set($cache_key, $rsWB,300);
	...
	public function edit_Webboard($data = array(), $post_id = 0) {
        if (!empty($data)) {
            $cache_key = "Webboard_Post_am_".$post_id;
			$this->tppymemcached->delete($cache_key);
			
			$this->db->where('wp.wb_post_id', $post_id);
            $this->db->update('webboard_post wp', $data);
            return ($this->db->affected_rows() != 1) ? false : true;
        } else {
            return $data['data']=NULL;
        }
    }
	public function create_Webboard($data = array()) {
        if (!empty($data)) {
            $this->db->insert('webboard_post', $data);
    ...
    //Json
    public function json_decode($url) {
    	$json_string=$url;
    	$jsondata = file_get_contents($json_string);
