<?php
class Subcategory_member_model extends CI_Model {
    ...
    public function get_status($id){
    	...
    	$this->db->from('_subcategory_member');
    ...
    public function getSelectSubcat($category_id = 0, $default = 0,$name = "subcategory_id"){
    	...
    	for($i=0;$i<count($rows);$i++){
    		$row = @$rows[$i];
    		$opt[]	= makeOption($row['subcategory_id_map'], $row['subcategory_name']);
    	}
    ...
    public function get_subcategory($category_id=0, $subcategory_id=null, $subcat_status=null, $order='order_by', $order_type='Asc', $limit_start = 0, $listpage = 20){
		...
		$this->db->select('_subcategory_member.subcategory_id');
		$this->db->select('_subcategory_member.subcategory_id_map');
		$this->db->select('_subcategory_member.subcategory_name');
		$this->db->select('_subcategory_member.lang');
		$this->db->select('_subcategory_member.order_by');
		$this->db->select('_subcategory_member.create_date');
		$this->db->select('_subcategory_member.status');
		$this->db->from('_subcategory_member');
    ...
    function store_product($data){
		$insert = $this->db->insert('_subcategory_member', $data);
	    return $insert;
	}
    ...
    function update_subcategory($subcategory_id, $data){
		...
		$this->db->where('subcategory_id', $subcategory_id);
		$this->db->update('_subcategory_member', $data);
		$report = array();
	...
	function update_orderid_to_down($order, $max){
		...
		$this->db->update('_subcategory_member');
		//Debug($this->db->last_query());
	...
	function delete_subcategory_by_admin($subcategory_id){
		$this->db->where('subcategory_id', $subcategory_id);
		$this->db->delete('_subcategory_member'); 
	}
