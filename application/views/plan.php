...
<?php if($admin_type==1){?>
								<!-- PAGE CONTENT BEGINS -->
								<button class="btn btn-sm btn-primary" onclick="window.location='<?php echo site_url('tmon/plan/add') ?>';">
										
										<i class="clip-cog-2"></i>
										<?php echo $language['add'] ?> <?php echo ' '; ?><?php echo $language['plan'] ?> 
...
<?php
	...
	echo form_open('tmon/plan', $attributes);
?>
...
<div>
	...
	<thead>
			<tr>
				...
				<th width="333"><?php echo $language['name']?></th>
				<th width="329"><?php echo $language['plan']?></th>
				<th width="98"><?php echo $language['status']?> </th>
				<th width="283"class="hidden-480"><i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i><?php echo $language['date']?></th>
				<th width="102"><?=$language['settings']?></td>			</tr>
				  </thead>
				<tbody>
<?php
				...
				for($key=0;$key<$maxcat;$key++){
					$plan_id_map=$plan_list[$key]['plan_id_map'];
					$plan_name=$plan_list[$key]['plan_name'];
					//$file=(int)$plan_list[$key]['file'];
					$status=$plan_list[$key]['status'];
					$date=$plan_list[$key]['date'];
?>
		<tr>
						<td><a href="#"><?=$plan_list[$key]['plan_id_map']?></a></td>
						<td><?php echo $i;?></td>
						<td><span class="badge badge-danger">  <?php $plan_name=$plan_list[$key]['plan_name']; echo "<b><font color='white'> $plan_name </font></b>";?> </span> </td>
						<td>
<?php  $file=$plan_list[$key]['file']; ?>					 
 <div id='admin_avatar'><span class="profile-picture">
...
<label>
<input name="status[]" id="status<?php echo $plan_id_map; ?>" class="ace ace-switch ace-switch-5 btn-empty" type="checkbox" <?php if($status=='1') echo 'checked';?>  value=1><span class="lbl"></span></label>
</label> 
<?php }else{ ?>
													...
													<div class="btn-group">
														...
														<ul role="menu" class="dropdown-menu pull-right">
															...
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="<?php echo site_url('tmon/plan/edit/'.$plan_list[$key]['plan_id_map']); ?>">
																	<i class="fa fa-edit"></i> <?php $edit=$language['edit']; echo "<b><font color='blue'> $edit </font></b>";?>
																</a>															</li>
														 <?php if($admin_type==1){?>
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="#"id="bx-confirm<?=$plan_list[$key]['plan_id_map']?>" class="tooltip-error" data-rel="tooltip" title="Delete">
...
?>
		$('#status<?=$plan_list[$key]['plan_id_map']?>').on('click', function() {
				var id = $(this).attr('id');
		    	var n = id.length;
			    var maxstr = n-6;
				...
				$.ajax({
					type: 'POST',
					url:  '<?php echo base_url('plan/status/'.$plan_list[$key]['plan_id_map'])?>',
					cache: false,
					success: function(data){
							if(data == 0){
                                ////////
                            	swal({
                            		title: "<?php echo $plan_name; echo ' ';echo $language['inactive'] ?>!",
                                ...
                                ////////
                            	swal({
                            		title: "<?php echo $plan_name;  echo ' ';echo $language['active'] ?>!",
                                    text: "<?php echo $language['savecomplate'] ?>", 
                            		timer: 2000,
                            		showConfirmButton: false
                            	});
		...
		$('#bootbox-confirm<?=$plan_list[$key]['plan_id_map']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('plan/delete/'.$plan_list[$key]['plan_id_map'])?>';
						}
					});
		});

		$('#bx-confirm<?=$plan_list[$key]['plan_id_map']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('plan/delete/'.$plan_list[$key]['plan_id_map'])?>';
						}
