...
<div class="col-xs-12"> 
								<!-- PAGE CONTENT BEGINS -->
								<button class="btn btn-sm btn-primary" onclick="window.location='<?php echo site_url('tmon/smsseting/add') ?>';">
										
										<i class="clip-cog-2"></i>
										<?php echo $language['add'] ?> <?php echo ' '; ?><?php echo $language['smsseting'] ?> 
...
<?php
	...
	echo form_open('tmon/smsseting', $attributes);
?>
...
<div>
	...
	<thead>
		<tr>
														...
														<?php echo $language['fullname'] ?></th>
														<th width="28%"><i class="ace-icon fa fa-sitemap bigger-110 hidden-480"></i>
														<?php echo $language['phone'] ?>  </th>
														<th width="20%">
...
<?php
				...
				$sms_id=$smsseting_list[$key]['sms_id'];
				$sms_id_map=$smsseting_list[$key]['sms_id_map'];
				$name=$smsseting_list[$key]['name'];
				$number=$smsseting_list[$key]['number'];
				$status=$smsseting_list[$key]['status'];
				$create_date=$smsseting_list[$key]['create_date'];
...
?>
		<tr>
						...
						<?php echo $sms_id_map;?><input type="hidden" class="ace" name="selectid[]" value="<?php echo $sms_id_map;?>"/>
						</span>
						</td>
						<td><?=$i?> </td>
...
?>
						...
						<td>
								<label> 
										<input name="status[]" id="status<?=$smsseting_list[$key]['sms_id_map']?>" class="ace ace-switch ace-switch-5 btn-empty" type="checkbox" <?php if($status== 1) echo 'checked';?>  value=1>
										<span class="lbl"></span>
								</label>
						</td>
						<td class="center">
						<?php #########################?>		
												<div>
													<div class="btn-group">
														...
														<?php if($smsseting_list[$key]['status'] == 1){?>
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="<?php echo site_url('tmon/smsseting/edit/'.$smsseting_list[$key]['sms_id_map']); ?>">
																	<i class="fa fa-edit"></i> <?php $edit=$language['edit']; echo "<b><font color='blue'> $edit </font></b>";?>
																</a>
															</li>
														 <?php }if($smsseting_list[$key]['status'] == 0){#########################?>
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="#"id="bx-confirm<?=$smsseting_list[$key]['sms_id_map']?>" class="tooltip-error" data-rel="tooltip" title="Delete">
...
<?php
		if($smsseting_list)		
				for($key=0;$key<$maxcat;$key++){		
						$title = $name;
?>
		$('#status<?=$smsseting_list[$key]['sms_id_map']?>').on('click', function() {
				var id = $(this).attr('id');
		    	var n = id.length;
			    var maxstr = n-6;
				...
				$.ajax({
					type: 'POST',
					url:  '<?php echo base_url('smsseting/status/'.$smsseting_list[$key]['sms_id_map'])?>',
					cache: false,
		...
		$('#bootbox-confirm<?=$smsseting_list[$key]['sms_id_map']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('smsseting/delete/'.$smsseting_list[$key]['sms_id_map'])?>';
						}
					});
		});

		$('#bx-confirm<?=$smsseting_list[$key]['sms_id_map']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('smsseting/delete/'.$smsseting_list[$key]['sms_id_map'])?>';
						}
