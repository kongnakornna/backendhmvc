...
<div class="col-xs-12"> 
								...
								<button class="btn btn-sm btn-primary" onclick="window.location='<?php echo site_url('tmon/hardwaretestsetting/add') ?>';">
								<i class="clip-cog-2"></i><?php echo $language['add'] ?> <?php echo ' '; ?><?php echo $language['hardwaretestsetting'] ?> 
								</button>
								<?php }?>
<!-- ##########################  -->
...
<?php
	...
	echo form_open('tmon/hardwaretestsetting', $attributes);
?>
...
<div>
	...
	<thead>
		<tr>
														...
														<th width="20%"><i class="ace-icon fa fa-sort-amount-desc bigger-110 hidden-480"></i>
														<?php echo $language['title'] ?><?php #echo $language['hardwaretest'] ?></th>
														<!--<th width="10%"><?php #echo $language['electricitytype'] ?></th>
														<th width="9%"><?php #echo $language['waterpipe'] ?></th>-->
														<th width="11%">
														<i class="ace-icon fa fa-tint bigger-110 hidden-480"></i>
														<?php echo $language['waterpump'] ?></th>
														<th width="9%"><?php echo $language['flow'] ?></th>
														<th width="9%"><?php echo $language['current'] ?></th>
														<th width="11%"><?php echo $language['voltage'] ?></th>
														<th width="10%"><?php echo $language['power'] ?></th>
														<th width="8%"><?php echo $language['control'] ?></th>
														<!--
														<th width="20%">
														<i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>
														<?php  echo $language['date'] ?></th>
														-->
...
<?php
				...
				for($key=0;$key<$maxcount;$key++){
					...
					$control=$this->Hardwaretestsetting_model->get_hardwareapi($control_id,$status=1);
					//echo '$power_id='.$power_id;Debug($power);die();
					$control_name=$control[0]['hardware_name'];
					$control_ip=$control[0]['hardware_ip'];
					####################
													...
													<div class="btn-group">
														...
														<?php if($hardware_list[$key]['status'] == 1 || $hardware_list[$key]['status'] == 0){?>
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="<?php echo site_url('tmon/hardwaretestsetting/edit/'.$hardware_list[$key]['hardwaretest_id_map']); ?>">
...
?>
		$('#status<?=$hardware_list[$key]['hardwaretest_id_map']?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url:  '<?php echo base_url('hardwaretestsetting/status/'.$hardware_list[$key]['hardwaretest_id_map'])?>',
					cache: false,
		...
		$('#bootbox-confirm<?=$hardware_list[$key]['hardwaretest_id_map']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('hardwaretestsetting/delete/'.$hardware_list[$key]['hardwaretest_id_map'])?>';
						}
					});
		});
