...
<?php 
		...
		$i=0;
		$maxcat = count($emailalertlog_list);
		$lang=$language['lang']; 
?>
<style type="text/css">
...
<?php
				...
				$i=1;
				//$i=0;
			if($emailalertlog_list){
			$data=$emailalertlog_list;
...
<?php
		if($emailalertlog_list)		
				for($key=0;$key<$maxcat;$key++){
						...
						$sensor_name = $emailalertlog_list[$key]['sensor_name'];
						$mail_id = $emailalertlog_list[$key]['mail_id'];
?>
		$('#status<?=$mail_id?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('emailalertlog/status/'.$mail_id)?>",
					cache: false,
