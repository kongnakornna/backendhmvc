...
<?php
		...
		echo form_open('tmon/callsetup/save', $attributes);
?>
									<div class="page-header">
										<h1>
											<a href="<?php echo base_url($this->uri->segment(1)) ?>"><?php echo $language['callsetup'] ?></a>
											<small>
...
<?php
			if(function_exists('Debug')){
					...
					//Debug($callsetup);Die();
					$countitem = count($callsetup);
					//echo'$countitem='.$countitem;Die();
					$condition_group_id2=$callsetup[0]['condition_group_id2'];
  					$condition_group_name_en=$callsetup[0]['condition_group_name'];
                    $condition_group_name_th=$callsetup[1]['condition_group_name'];
                    $hour_start=$callsetup[0]['hour_start'];
                    $minute_start=$callsetup[0]['minute_start'];
                    $hour_finish=$callsetup[0]['hour_finish'];
                    $minute_finish=$callsetup[0]['minute_finish'];
                    $day_start=$callsetup[0]['day_start'];
                    $day_finish=$callsetup[0]['day_finish'];
                    $date_start=$callsetup[0]['date_start'];
                    $date_finish=$callsetup[0]['date_finish'];
                    $month_start=$callsetup[0]['month_start'];
                    $month_finish=$callsetup[0]['month_finish'];
                    $Sun=$callsetup[0]['Sun'];
					$Mon=$callsetup[0]['Mon'];
					$Tue=$callsetup[0]['Tue'];
					$Wed=$callsetup[0]['Wed'];
...
?>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input name="condition_group_name_th" type="text" class="col-xs-10 col-sm-5" id="condition_group_name_th" value="<?php echo $condition_group_name_th;?>" placeholder="<?php echo $language['title']?> <?php echo $language['callsetup'] ?> (TH)"> 
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" class="col-xs-10 col-sm-5" placeholder="<?php echo $language['title']?> <?php echo $language['callsetup'] ?> (EN)" id="condition_group_name_en" name="condition_group_name_en" value="<?php echo $condition_group_name_en;?>"> 
...
?>
		$('#status<?=$admin_team[$key]['admin_team_id']?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('callsetup/status/'.$admin_team[$key]['admin_team_id'])?>",
					cache: false,
		...
		$('#bootbox-confirm<?=$admin_team[$key]['admin_team_id']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('callsetup/delete/'.$admin_team[$key]['admin_team_id'])?>';
						}
					});
		});
