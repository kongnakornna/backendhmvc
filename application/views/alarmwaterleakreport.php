...
<?php 
		...
		$lang=$language['lang']; 
		$erroralert=$this->lang->line('error');
?>
<style type="text/css">
...
?>
<!-- ##########################  -->
<center> <div class="panel-body"style="padding: 1px 5px;"><?php  echo $pagination;?></div>  </center>
<!-- ##########################  -->
										<div class="row">
...
<?php
	...
	echo form_open('tmon/alarmwaterleakreport', $attributes);
?>
<div class="row"> <div class="col-md-12 space20"> <div class="btn-group pull-right">
<?php echo $this->lang->line('startdate');?>&nbsp;
<input name="startdate" type="text" class="some_class" id="some_class_1" value="<?php echo $startdate;?>"/>
<?php echo $this->lang->line('to');?> &nbsp;
<input name="enddate" type="text" class="some_class" id="some_class_2" value="<?php echo $enddate; ?>"/>
&nbsp;&nbsp;
	...
	<table>-->
		<thead>
			<tr>
				...
				<th width="221"><?php echo $language['sensor'] ?></th>
				<th width="298"><?php echo $language['status'] ?></th>
				<th width="229"> <?php echo $language['sensorvalue']?> </th>
				<th width="256"> 
				<i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i><?php echo $language['datetime'] ?>
...
<?php
				...
				for($key=0;$key<$maxcat;$key++){
					$wtl_id=$web_menu[$key]['wtl_id'];
					$wtl_name=$web_menu[$key]['wtl_name'];
					$wtl_statusname=$web_menu[$key]['wtl_statusname'];
					$wtl_datetimelog=$web_menu[$key]['wtl_datetimelog'];
					$wtl_status=$web_menu[$key]['wtl_status'];
?>

		<tr>
 						<td><a href="#"><?=$wtl_id?></a></td>
						<td><?php echo $i;?></td>
						<td><a href="#"><?=$web_menu[$key]['wtl_name']?></a></td>
						<td><a href="#"> 
						<?php 
						$wtl_statusname=$web_menu[$key]['wtl_statusname'];
						$alert_status=$web_menu[$key]['wtl_status'];
						if($alert_status=='Alert'){
						echo "<b><font color='red'> $wtl_statusname</font></b>";
						}else if($alert_status=='Normal'){
						echo "<b><font color='green'> $wtl_statusname </font></b>";
						}else{echo $alert_status;}
						?>
						</a></td>
						<td><a href="#"><?php 
						$alert_status=$web_menu[$key]['wtl_status'];
						if($alert_status=='Alert'){
						echo "<b><font color='red'> $alert_status</font></b>";
						}else if($alert_status=='Normal'){
						echo "<b><font color='green'> $alert_status </font></b>";
						}else{echo $alert_status;}
						?>
						</a></td>
						<td><a href="#"><?php $create_date=$web_menu[$key]['wtl_datetimelog'];
...
<?php
		if($web_menu)		
				for($key=0;$key<$maxcat;$key++){
						...
						$sensor_log_id = $web_menu[$key]['sensor_log_id'];
?>
		$('#status<?=$sensor_log_id?>').on('click', function() {
				var id = $(this).attr('id');
		    	var n = id.length;
			    var maxstr = n-6;
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('alarmlogreport/status/'.$sensor_log_id)?>",
					cache: false,
		...
		$('#bootbox-confirm<?=$sensor_log_id?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('admin_menu/delete/'.$sensor_log_id)?>';
						}
					});
		});

		$('#bx-confirm<?=$sensor_log_id?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('admin_menu/delete/'.$sensor_log_id)?>';
						}
