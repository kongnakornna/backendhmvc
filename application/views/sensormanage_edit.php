...
<?php
		...
		echo form_open('tmon/sensormanage/save', $attributes);
?>
									<div class="page-header">
										<h1>
											<a href="<?php echo base_url($this->uri->segment(1)) ?>"><?php echo $language['sensormanage'] ?></a>
											<small>
...
<?php
			if(function_exists('Debug')){
					...
					//Debug($sensormanage);Die();
					$countitem = count($sensormanage);
					//echo'$countitem='.$countitem;Die();
					$condition_group_id2=$sensormanage[0]['condition_group_id2'];
  					$condition_group_name_en=$sensormanage[0]['condition_group_name'];
                    $condition_group_name_th=$sensormanage[1]['condition_group_name'];
                    $hour_start=$sensormanage[0]['hour_start'];
                    $minute_start=$sensormanage[0]['minute_start'];
                    $hour_finish=$sensormanage[0]['hour_finish'];
                    $minute_finish=$sensormanage[0]['minute_finish'];
                    $day_start=$sensormanage[0]['day_start'];
                    $day_finish=$sensormanage[0]['day_finish'];
                    $date_start=$sensormanage[0]['date_start'];
                    $date_finish=$sensormanage[0]['date_finish'];
                    $month_start=$sensormanage[0]['month_start'];
                    $month_finish=$sensormanage[0]['month_finish'];
                    $Sun=$sensormanage[0]['Sun'];
					$Mon=$sensormanage[0]['Mon'];
					$Tue=$sensormanage[0]['Tue'];
					$Wed=$sensormanage[0]['Wed'];
					$Thu=$sensormanage[0]['Thu'];
					$Fri=$sensormanage[0]['Fri'];
					$Sat=$sensormanage[0]['Sat'];
					$status=$sensormanage[0]['status'];
...
?>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input name="condition_group_name_th" type="text" class="col-xs-10 col-sm-5" id="condition_group_name_th" value="<?php echo $condition_group_name_th;?>" placeholder="<?php echo $language['title']?> <?php echo $language['sensormanage'] ?> (TH)"> 
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" class="col-xs-10 col-sm-5" placeholder="<?php echo $language['title']?> <?php echo $language['sensormanage'] ?> (EN)" id="condition_group_name_en" name="condition_group_name_en" value="<?php echo $condition_group_name_en;?>"> 
											 <?php  //echo 'sensormanage Control Auto works';//echo $language['hwcontrols']; ?>
										</div>
									</div>
...
?>
		$('#status<?=$admin_team[$key]['admin_team_id']?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('sensormanage/status/'.$admin_team[$key]['admin_team_id'])?>",
					cache: false,
		...
		$('#bootbox-confirm<?=$admin_team[$key]['admin_team_id']?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('sensormanage/delete/'.$admin_team[$key]['admin_team_id'])?>';
						}
					});
		});
