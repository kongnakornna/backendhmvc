...
*/
class Team1 extends MY_Controller {
...
$total_rows= $this->Admin_team_model->totalteam($startdate,$enddate);
		...
		$pageConfig = doPagination($this->Admin_team_model->totalteam($startdate,$enddate), $limit, $segment, site_url("/team/listview/"));
		}
		//Debug($pageConfig);
		//die();
	...
	public function status($id){
		...
		//$id = $this->input->post("id");
			...
			$log_activity = array(
								...
								"ref_id" => $id,
								"ref_type" => 0,
								"ref_title" => "Admin team : ".$admin_team_name." [Status]",
								"action" => 2
			);			
	...
	public function save(){
		...
		if ($this->form_validation->run()) 
			...
			echo "<script>alert('Save Data and activity log');window.location.href='team';</script>";
			// redirect alert After
			//echo "<script> window.location.href='team'; alert('Save Data'); </script>";
			// non alert 
			//redirect('team');
