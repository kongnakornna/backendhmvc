											...
											<table width="100%" class="table table-striped table-bordered table-hover" id="sample-table-2">
												<thead>
													<tr>
														...
														<th width="2%"class="hidden-480"><?php echo $language['no'] ?></th>
														<!--
														<th width="2%"class="hidden-480">ID</th>
														<?php if($this->session->userdata('admin_id') == 1){ ?>
														<th width="2%"class="hidden-480">ID2</th>
														<?php } ?>
														-->
														<th width="52%"><?php echo $language['name'] ?></th>
														<th width="6%"class="hidden-480"><?php echo $language['submenu'] ?></th>
														...
														<th width="12%" class="hidden-480">
															<i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>
															<?php echo $language['create_date'] ?>														</th>
														<th width="6%"><?php echo $language['status'] ?></th>
...
?>
		<tr>
						...
						<!--
						<td class="hidden-480"><?=$web_menu[$key]['admin_menu_id']?><input type="hidden" class="ace" name="selectid[]" value="<?=$menu_id?>" /></td>
						<?php if($this->session->userdata('admin_id') == 1){ ?>
						<td class="hidden-480"><?=$menu_id?></td>
						<?php } ?>
						-->
						<td><a href="<?php echo site_url('admin_menu/view/'.$menu_id); ?>">
						
<?php $count_sub=$web_menu[$key]['count_sub']; ?>						 
<i class="fa <?=$web_menu[$key]['icon']?>"></i>
<?php echo $web_menu[$key]['title'];?>

<?php if($count_sub>5){?>
<span class="badge badge-success"> <?php echo $count_sub;?></span>
<?php }elseif($count_sub==5||$count_sub==4||$count_sub==3||$count_sub==2||$count_sub==1){?>
<span class="badge badge-warning"> <?php echo $count_sub;?></span>
<?php }else if($count_sub<1){?>
<span class="badge badge-danger"> <?php echo $count_sub;?></span>
<?php }?>
						</a></td>
						<td class="hidden-480"><span class="label label-success"> <?=$web_menu[$key]['count_sub']?> </span></td>
						<td class="hidden-480">
								 <input class="input-small" name="order[]" type="text" id="order_by" placeholder="Not null" value="<?=$web_menu[$key]['order_by']?>">
								<!-- &nbsp;<i class="fa fa-arrow-up"></i>&nbsp;
								<i class="fa fa-arrow-down"></i> -->
						</td>
						<td class="hidden-480"><span class="label label-success"><?=$web_menu[$key]['create_date']?> </span></td>
						<td>
								<!-- <span class="col-sm-6">
