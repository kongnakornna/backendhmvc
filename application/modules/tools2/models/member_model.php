...
class Member_model extends CI_Model {
  ...
  private $user_status = array(
    'th'=> array(
      '0'=> 'กรุณายืนยันการสมัครสมาชิกของท่าน หรือ <a href="http://www.trueplookpanya.com/member/activate/">กดที่นี่</a>เพื่อส่งอีเมลยืนยันการสมัครอีกครั้ง',
      '2'=> 'ไอดีของท่านโดนระงับการใช้งานชั่วคราว กรุณาส่งอีเมลมาที่ admin@trueplookpanya.com ค่ะ',
      '3'=> 'ระบบขัดข้องโปรดลองใหม่อีกครั้งค่ะ',
    ),
    'en'=> array(
      '0'=> 'Please Activate Email',
      '2'=> 'Your account is disabled. Please contract : admin@trueplookpanya.com',
  ...
  function get_profile_by_user_id_short($user_id=0){
    $key = "member_short_profile_" . $user_id;
    // _vd($key);
     // $this->tppymemcached->delete($key);
    // if (!$member = $this->tppymemcached->get($key)) {
      $sql = "SELECT u.user_id ,u.user_username ,u.psn_display_name ,u.psn_firstname ,u.psn_lastname ,u.user_email ,u.psn_display_image 
  ...
  public function check_user_status($profiles=array(), $lang='th'){
    if (count($profiles) == count($profiles, COUNT_RECURSIVE)) {
      $_temp[] = $profiles;
      unset($profiles);
      $profiles = $_temp;
    }
    ...
    if($profiles){
      foreach($profiles as $profile) {
        ...
        if($user_status == '1') { // ปกติ
          return $profile;
        } else if($user_status  == '0') { // ยังไม่ได้ active email
          $data['error']=$this->user_status[$lang][0];
        } else if($user_status == '2'){ // โดนแบน
          $data['error']=$this->user_status[$lang][2];
        } else { // โดนลบ หรือระบบมีปัญหา
          $data['error']=$this->user_status[$lang][3];
        }
      }
    }
     ...
     if($query->num_rows()) {
        ...
        foreach($result as &$rs) {
          if(empty($result['blog_id'])){
            $blog_id = $this->insert_blog($rs['user_id'], $rs['member_id']);
            $rs['blog_id'] = $blog_id;
          }
        }
  ...
  function get_profile_by_userid($user_id, $clear_cache=FALSE) {
    // $key = "memberprofile_" . $user_id;
    // if($clear_cache){
      // $this->tppymemcached->delete($key) ;
    // }
