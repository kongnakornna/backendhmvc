<?php 
		...
		$caption = (trim($picture_list[$i]['caption']) == '') ? $instagram_list['title'] : $picture_list[$i]['caption'];
		$instagram_id = $this->input->get('instagram_id');
?>
<style type="text/css">
...
<div class="row">
	<div class="col-xs-12">
			...
			<button class="btn btn-danger btn-sm btn-primary" onclick="window.location='<?php echo site_url('instagram/picture/'.$instagram_id) ?>';">
				<i class="bace-icon fa fa-reply icon-only igger-125"></i><?php echo $language['backto'].' '.$language['picture']  ?>
			</button>

			<!-- <button class="btn  btn-sm btn-primary" onclick="window.location='<?php echo site_url('instagram/picture_edit/'.$this->uri->segment(3).'?instagram_id='.$instagram_id.'&Orientation=2') ?>';">
	...
	<div class="col-xs-12">
		...
		<div class="col-lg-6">
			<!-- <button class="btn btn-sm btn-info" onclick="window.location='<?php echo site_url('instagram/rotate/'.$this->uri->segment(3).'?instagram_id='.$instagram_id.'&folder='.$folder.'&rotate=l&file='.$picture_list[$i]['file_name']) ?>';">
				<i class="bace-icon fa fa-circle-o icon-only igger-125"></i><?php echo $language['rotate'].' '.$language['left']  ?>
			</button>
			<button class="btn btn-sm btn-info" onclick="window.location='<?php echo site_url('instagram/rotate/'.$this->uri->segment(3).'?instagram_id='.$instagram_id.'&folder='.$folder.'&rotate=r&file='.$picture_list[$i]['file_name']) ?>';">
				<i class="bace-icon fa fa-circle-o icon-only igger-125"></i><?php echo $language['rotate'].' '.$language['right']  ?>
			</button> -->
		</div>
<?php
		$attributes = array('class' => 'form-horizontal', 'id' => 'jform');
		echo form_open_multipart('instagram/picture_watermark', $attributes);
					...
					if($picture_list[$i]['file_name'] != ''){
							...
							$file_name = 'uploads/instagram/'.$folder.'/'.$file_org;
					...
					if(!$imgsize){
						...
						$file_nameorg = 'uploads/instagram/'.$folder.'/'.$file_org;
			...
			$img_org = 'uploads/instagram/'.$folder.'/'.$file_org;
...
<div class="col-lg-6">
		...
		<input type="hidden" name="instagram_id" value="<?php echo $instagram_id?>">
...
function crop_image34(){
			...
			Waiting();
						$.ajax({
							type:"GET",
							url:"<?php echo base_url()?>picture/make_img?type=instagram&img="+$("#photo").attr('data-src')+"&w=" + $("#width").val() + "&h="+ $("#height").val() + "&x1=" + $("#left").val() + "&y1=" + $("#top").val() + "&folder=" +$("#photo").attr('data-folder'),
							cache:false,
							success:function(rsponse){
...
?>	
	...
	$('#button_rotateL').click(function(){
		...
		$.ajax({
		   type: "POST",
		   url: "<?php echo site_url('instagram/rotate/'.$this->uri->segment(3).'?folder='.$folder.'&rotate=l&file='.$picture_list[0]['file_name']) ?>",
		   //url: "<?php echo site_url('instagram/rotate') ?>",
		   //data: "instagram_id : <?php //echo $instagram_id?>, rotate : 'l', folder : '<?php echo $folder?>', file : '<?php //echo $picture_list[0]['file_name']?>' ",
	...
	$('#button_rotateR').click(function(){
		...
		$.ajax({
		   type: "POST",
		   url: "<?php echo site_url('instagram/rotate/'.$this->uri->segment(3).'?folder='.$folder.'&rotate=r&file='.$picture_list[0]['file_name']) ?>",
		   //url: "<?php echo site_url('instagram/rotate') ?>",
		   //data: "instagram_id : <?php //echo $instagram_id?>, rotate : 'r', folder : '<?php echo $folder?>', file : '<?php //echo $picture_list[0]['file_name']?>' ",
		   success: function(msg){
	     		$('#save_results').html(msg);
				//alert(msg);
				window.location='<?php echo "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>';
	...
	$('#delete-hl').click(function(){
			...
			$.ajax({
				   type: "POST",
				   url: "<?php echo site_url('instagram/remove_pic') ?>",
				   data: {src: v},
				   success: function(msg){
