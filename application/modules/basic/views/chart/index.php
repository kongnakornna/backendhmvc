<?php
		...
		$donut = $area_chart = '';
		if($report_morris->name)
			for($i=0;$i<count($report_morris->name);$i++){
					$donut .= '{ label: "'.$report_morris->name[$i].'", value: '.$report_morris->sum_view[$i].'},';
			}
		...
		if($lineall->view_date)
			for($i=0;$i<count($lineall->view_date);$i++){
					...
					if($i == 0) 
						$area_chart .= '{ period: "'.$period.'", desktop: '.$desktop.', mobile: '.$mobile.', tablet: '.$tablet.'}';
					else
						$area_chart .= ',
						{ period: "'.$period.'", desktop: '.$desktop.', mobile: '.$mobile.', tablet: '.$tablet.'}';
			}
?>
<style type="text/css">
		...
		<div class="row">
                ...
                <!-- <div class="col-lg-6"> -->
					<div class="vspace-12-sm"></div>
						<?php $this->chart_model->Traffic_sources($report) ?>
