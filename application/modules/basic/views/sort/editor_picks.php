<?php 
	...
	$opt = array();
	$tmp = 0;
	$gen_url = base_url('json/gen_editor_picks');
	
	//Debug($editor_picks_list);
	//Debug($json_editor_picks);
...
<?php
	...
	//echo form_open_multipart('order/save_editor_picks', $attributes);
?>
			<div class="page-header">
					<h1>
							...
							<small>
									...
									<?php echo $language['editor_choice'] ?>
							</small>
					</h1>
			</div>
...
/*
$new_editor_picks = $editor_picks_list;
	if(isset($editor_picks_list)){
			$maxorder = count($editor_picks_list);
			for($i=0;$i<$maxorder;$i++){
					$number = $i+1;
					$new_editor_picks[$i] = $new_editor_picks[$i]->order;
...
<?php
								if(isset($editor_picks_list)){
										...
										for($i=0;$i<$maxorder;$i++){
												...
												if(isset($editor_picks_list[$i]->editor_picks_id)){
															
													$editor_picks_id = $editor_picks_list[$i]->editor_picks_id;
													$news_id = $editor_picks_list[$i]->article_id;
													$title = $editor_picks_list[$i]->title;

													$status = ($editor_picks_list[$i]->status == 1) ? 'Actived' : 'Inactived';
													$approve = ($editor_picks_list[$i]->approve == 1) ? 'approve' : 'no approved';
													...
													$button_del = '<a class="red del-confirm" href="javascript:void(0);" id="bootbox-confirm'.$editor_picks_id.'" data-value="'.$editor_picks_id.'" data-name="'.$title.'" data-rel="tooltip" title="Delete"><i class="ace-icon fa fa-trash-o bigger-130" data-rel="tooltip" title="'.$language['delete'].'"></i></a>';
										...
										<div class="dd" id="nestable">
											...
											<?php
												...
												if(isset($editor_picks_list))
														...
														for($i=0;$i<$maxorder;$i++){
														//foreach($editor_picks_list as $i => $obj){
															$number = $i + 1;
															$type = "";
															/*foreach($obj as $field => $val){
																	if($field == "news_editor_picks_id") $editor_picks_id = $val;
															...
															if(isset($editor_picks_list[$i]->editor_picks_id)){
																...
																$editor_picks_id = $editor_picks_list[$i]->editor_picks_id;
																$news_id = $editor_picks_list[$i]->article_id;
																$ref_type = $editor_picks_list[$i]->ref_type;
																$order = $editor_picks_list[$i]->order;
																$title = $editor_picks_list[$i]->title;

																$status = ($editor_picks_list[$i]->status == 1) ? 'Actived' : 'Inactived';
																$approve = ($editor_picks_list[$i]->approve == 1) ? 'approve' : 'no approved';

																$category_name = (isset($editor_picks_list[$i]->category_name)) ? $editor_picks_list[$i]->category_name : '';
																$subcategory_name = (isset($editor_picks_list[$i]->subcategory_name)) ? ' => '.$editor_picks_list[$i]->subcategory_name : '';

																$lastupdate_date = $editor_picks_list[$i]->lastupdate_date;
																$create_date = $editor_picks_list[$i]->create_date;
																$countview = $editor_picks_list[$i]->countview;
																$file_name = $editor_picks_list[$i]->file_name;
																$folder = $editor_picks_list[$i]->folder;

																$type = (isset($editor_picks_list[$i]->type)) ? $editor_picks_list[$i]->type : "Gallery";
																//if($type == "") $editor_picks_list[$i]->type = "Gallery";
																...
																//Debug($pathpic);
																$editor_picks_list[$i]->picture = $pathpic;
																//Debug($editor_picks_list[$i]);
																...
																//Debug($editor_picks_list[$i]);
																/*$editor_picks_id = $editor_picks_list[$i]->news_editor_picks_id;
																$news_id = $editor_picks_list[$i]->news_id;
																$ref_type = $editor_picks_list[$i]->ref_type;

																$order = $editor_picks_list[$i]->order;
																$title = $editor_picks_list[$i]->title;
																$category_name = $editor_picks_list[$i]->category_name;
																$lastupdate_date = RenDateTime($editor_picks_list[$i]->lastupdate_date);
																$create_date = $editor_picks_list[$i]->create_date;
																$countview = $editor_picks_list[$i]->countview;*/

																//list($create_date, $time) = explode(" ", $editor_picks_list[$i]->folder_news);
...
<?php 
		...
		//Debug($json_editor_picks);
		//Debug($json_editor_picks['body']['item']);

		@$default_order = (isset($json_editor_picks['body']['item'])) ? count($json_editor_picks['body']['item']) : 4;
?>
...
$( document ).ready(function() {
		...
		$('.del-confirm').on('click', function() {				
				...
				bootbox.confirm("<?php echo $language['are you sure to delete']?> <br><b>editor_picks id " + id + "</b> <span class='green'>" + title + "</span>", function(result) {
					if(result) {
							...
							title = title.replace("&", ""); 
							title = title.replace(".", ""); 
							//alert('<?php echo base_url('order/delete_editor_picks')?>/' + id + '/' + encodeURI(title));
							window.location='<?php echo base_url('order/delete_editor_picks')?>/' + id + '/' + encodeURI(title);
						}
				});
		});
...
?>
				//window.location.href ='<?php echo base_url('json/gen_editor_picks') ?>?number=' + list_order;
				WaitingAlert('<?php echo $language['waiting_for_generate']; ?>');
				gen_editor_picks(list_order);				
		});
...
function UpdateOrder(json){
			...
			$.ajax({
					type: 'POST',
					url: "<?php echo base_url('order/update_ed')?>",
					data: {json: json},
					cache: false,
...
function gen_editor_picks(list_order){

			//Gen Home Page
			$.getJSON( "<?php echo base_url('json/gen_editor_picks') ?>?number=" + list_order, function( rsponse ) {
					//alert(rsponse);
					if(rsponse.header.resultcode == 200){
							AlertSuccess	('Generate editor_picks success.');
							Gen_www();
					}else{
