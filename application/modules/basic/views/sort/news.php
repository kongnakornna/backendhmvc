											...
											<?php
												...
												if(isset($news_list))
														...
														for($i=0;$i<$maxorder;$i++){
															...
															if(($tmp+1) != $order){
																$order = $tmp+1;
																//$this->news_model->set_order(0, intval($order), $catid,$news_id);
																$redirect = $_SERVER["SERVER_NAME"].$_SERVER["REDIRECT_URL"];
															}

															$list_order = selectListOrder( $opt, 'news'.$news_id.'[]', 'list_order'.$news_id, 'class="input-small list_order" data-id="'.$news_id.'" data-number="'.$order.'" '.$disable, 'value', 'text', $order);
...
function UpdateNewsOrder(newsid, val){
				...
				$.ajax({
						...
						data : { newsid : newsid, val : val, catid : <?php echo $catid?>, pin_list : <?php echo json_encode($pin_list) ?>},
						cache: false,
						success: function(data){
