<?php 
		...
		$brand_id = 0;
		$category_iddb = 5;
		$allmagazine = count($magazine_list);
...
<?php 
				...
				echo form_open('magazine', $attributes);
?>
		<div class="col-xs-12 col-lg-3">
				<button type="button" class="btn btn-sm btn-primary" onclick="window.location='<?php echo site_url('magazine/add') ?>';">
					<i class="ace-icon glyphicon glyphicon-plus align-top bigger-125"></i><?php echo $language['addmagazine'] ?>
				</button> <?php echo $allmagazine." of ".number_format($countmagazine)." record." ?>
		</div>
...
<?php
	...
	echo form_open('magazine', $attributes);
?>									
											<table id="dataTables-magazine" class="table table-striped table-bordered table-hover">
												<thead>
													<tr>
														<th>No.</th>
														<th class="hidden-480">
...
<?php
			...
			if($magazine_list)
					for($i=0;$i<$allmagazine;$i++){
								...
								//$pic = ($magazine_list[$i]['avatar'] == '') ? 'theme/assets/avatars/avatar3.png' : 'uploads/magazine/'.$magazine_list[$i]['avatar'];
								//$thumb = $pic;
								$magazine_id = $magazine_list[$i]['magazine_id'];
								$magazine_id2 = $magazine_list[$i]['magazine_id2'];
								//$dara_id = $magazine_list[$i]['dara_id'];

								$brand_id = $magazine_list[$i]['brand_id'];
								$brand_name = $magazine_list[$i]['brand_name'];

								//$full_name = $magazine_list[$i]['full_name'];

								$title = $magazine_list[$i]['title'];
								//$description = $magazine_list[$i]['description'];
								$order_by = $magazine_list[$i]['order_by'];
								$create_date = $magazine_list[$i]['create_date'];
								//$create_date = RenDateTime($create_date);
								$create_by_name = $magazine_list[$i]['create_by_name'];

								$countview = $magazine_list[$i]['countview'];

								$can_comment = $magazine_list[$i]['can_comment'];

								$status = $magazine_list[$i]['status'];
								$approve = $magazine_list[$i]['approve'];
								//$is_18 = $magazine_list[$i]['is_18'];

								//$news_highlight_id = $magazine_list[$i]['highlight_id'];
								//$megamenu_id = $magazine_list[$i]['megamenu_id'];

								if($magazine_list[$i]['file_name'] != ''){
										$thumb_img = base_url('uploads/thumb300/').'/'.$magazine_list[$i]['folder'].'/'.$magazine_list[$i]['file_name'];
										if(file_exists('uploads/thumb300/'.$magazine_list[$i]['folder'].'/'.$magazine_list[$i]['file_name']))
						...
						/*$highlight = ($news_highlight_id > 0) ? '<i class="ace-icon fa fa-asterisk hand rmhighlight green" data-value="'.$magazine_id2.'" id="hl'.$magazine_id2.'" data-name="'.$title.'" data-rel="tooltip" title="'.$language['highlight'].'"></i>' : '<i class="ace-icon fa fa-asterisk hand sethighlight" data-value="'.$magazine_id2.'" id="hl'.$magazine_id2.'" data-name="'.$title.'" data-rel="tooltip" title="Set '.$language['highlight'].'"></i>';

						$megamenu = ($megamenu_id > 0) ? '<i class="ace-icon glyphicon glyphicon-picture hand rmmegamenu green" data-value="'.$magazine_id2.'" data-name="'.$title.'" data-cat="'.$brand_name.'"  data-catid="'.$brand_id.'" id="mg'.$magazine_id2.'" data-rel="tooltip" title="Mega menu"></i>' : '<i class="ace-icon glyphicon glyphicon-picture hand setmegamenu" data-value="'.$magazine_id2.'" data-name="'.$title.'" data-cat="'.$brand_name.'"  data-catid="'.$brand_id.'" id="mg'.$magazine_id2.'" data-rel="tooltip" title="Set Mega menu"></i>';*/
								
						$edit_magazine = site_url('magazine/edit/'.$magazine_id2);
						$pic_magazine = site_url('magazine/picture/'.$magazine_id2);
						$previewurl = $this->config->config['www']."/magazine/".$category_iddb."/".$brand_id."/".$magazine_id2."/?preview=1";
?>
		<tr>
						<!-- <td class="center">
						...
						<td><?=$magazine_id2?><input type="hidden" class="ace" name="selectid[]" value="<?php echo $magazine_id2?>" /></td>
						<td><a href="<?php echo $edit_magazine ?>"><?=$show_img?></a></td>
						<td>
								<a href="<?php echo $edit_magazine ?>"><?=$title?></a>
								<br>View : <?php echo number_format($countview) ?>
								<br><?php echo $language['create_by'] ?> : <?php echo $create_by_name?>
						</td>
						<td class="hidden-480"><?=$brand_name?></td>
						<td class="hidden-480"><?php echo $create_date ?></td>
						<td class=""><span class="col-sm-6">
								<label class="pull-right inline" id="enable<?php echo $magazine_id2?>">
								<small class="muted"></small>
									<input type="checkbox" id="status<?=$magazine_id2?>" class="ace ace-switch ace-switch-6 status-buttons" data-value="<?=$magazine_id2?>"
						...
						<td class=""><span class="col-sm-6">
								<label class="pull-right inline" id="enable<?php echo $magazine_id2?>">
								<small class="muted"></small>
									<input type="checkbox" id="approve<?=$magazine_id2?>" class="ace ace-switch ace-switch-4 approve-buttons" data-value="<?=$magazine_id2?>"
						...
						<td>
												...
												<div class="hidden-sm hidden-xs action-buttons">
															...
															<a href="<?php echo $pic_magazine ?>" data-rel="tooltip" title="Picture">
																	<i class="menu-icon fa fa-picture-o"></i>
															</a>
															<a class="green" href="<?php echo $edit_magazine ?>" data-rel="tooltip" title="Edit">
																	<i class="ace-icon fa fa-pencil bigger-130"></i>
															</a>
															<a class="red del-confirm" href="javascript:void(0);" id="bootbox-confirm<?php echo $magazine_id2?>" data-value="<?=$magazine_id2?>" data-cat="<?=$brand_id?>" data-name="<?=$title?>" data-rel="tooltip" title="Delete">
															...
															<div class="hidden-md hidden-lg">
																<div class="inline position-relative">
																	...
																	<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																		...
																		<li>
																			<a href="<?php echo $pic_magazine ?>" class="tooltip-info" data-rel="tooltip" title="Picture">
																				<span class="blue">
																		...
																		<li>
																			<a href="<?php echo $edit_magazine ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
																				<span class="green">
																		...
																		<li>
																			<a href="javascript:void(0);" id="bx-confirm<?php echo $magazine_id2?>" class="tooltip-error del-confirm" data-value="<?=$magazine_id2?>" data-cat="<?=$brand_id?>" data-name="<?=$title?>" data-rel="tooltip" title="Delete">
																				<span class="red">
...
$( document ).ready(function() {
		...
		$('.status-buttons').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('magazine/status')?>/" + magazineid,
					cache: false,
...
?>
						$.ajax({
							type: 'POST',
							url: "<?php echo base_url('magazine/approve')?>/" + magazineid,
							cache: false,
							success: function(data){
									//alert(data);
									if(data == 0){
		...
		$('.del-confirm').on('click', function() {
				...
				bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
					if(result) {
							...
							window.location='<?php echo base_url('magazine/delete')?>/' + id + '?cat=' + catid;
						}
				});
		});
		...
		$('.sethighlight').on('click', function() {				
				...
				bootbox.confirm("<h4 class='row header smaller lighter purple'>Set Highlight </h4>" + title , function(result) {
					if(result) {
							$.ajax({
										type: 'POST',
										url: "<?php echo base_url('magazine/sethighlight')?>/" + id,
		...
		$('.setmegamenu').on('click', function() {				
				...
				bootbox.confirm("<h4 class='row header smaller lighter orange'>Set Megamenu </h4>" + title , function(result) {
					if(result) {
							$.ajax({
										type: 'POST',
										url: "<?php echo base_url('magazine/setmegamenu')?>/" + id,
