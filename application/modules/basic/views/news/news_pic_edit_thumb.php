...
<?php
		...
		echo form_open('news/picture_watermark', $attributes);
					...
					$img_org = 'uploads/news/'.$folder.'/'.$file_org;							//620x349
					$img_headnews = 'uploads/headnews/'.$folder.'/'.$file_org;		//620x349
			...
			<div class="<?php echo $col_size2?> ace-thumbnails">
						...
						<div id="thumbs4">
								<?php if(file_exists($thumbs4)){ 
									...
									Debug('<img src='.base_url($thumbs4).' ><br>'.$imagesize[0].'x'.$imagesize[1]); } ?>
						</div>
...
function crop_image34(){
						...
						//alert("<?php echo base_url()?>picture/make_img?type=news&img="+$("#photo").attr('data-src')+"&w=" + $("#width").val() + "&h="+ $("#height").val() + "&x1=" + $("#left").val()  + "&y1=" + $("#top").val() + "&folder=" +$("#photo").attr('data-folder'));
...
<?php
			...
			$cur_url .= $cur_url."?news_id=".$news_id;
?>
	...
	$('#delete-hl').click(function(){
			...
			$.ajax({
				   type: "POST",
				   url: "<?php echo site_url('news/remove_pic') ?>",
				   data: {src: v},
				   success: function(msg){
