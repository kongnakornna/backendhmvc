			...
			if($item_list)
					for($i=0;$i<$allnews;$i++){
								...
								if($ref_type == 1){
										...
										//$file_name = ($item_list[$i]->file_name == '') ? 'uploads/news/'.$folder.'/'.$item_list[$i]->file_name : 'uploads/thumb/'.$folder.'/'.$item_list[$i]->file_name;
										...
										$file_name = Img_Youtube($youtube_url);
										//$alink = $thumb_youtube;
								}
								
								//$pic_del = base_url('article/picture_del/'.$galleryset_id)."?".$this->uri->segment(3);
								$attr_width = 'width="120" height="68"';
								//$edit_picture = base_url('article/picture_edit/'.$galleryset_id.'?news_id='.$this->uri->segment(3));
		...
		<div class="col-md-offset-1 col-md-9">
				<a href="<?php echo base_url('article/gallery/'.$this->uri->segment(3))?>">
					<button type="button" class="btn btn-danger">
						<i class="ace-icon fa fa-reply icon-only bigger-110"></i> <?php echo $language['back']?>
					</button>
				</a>
		</div>
	</div>
