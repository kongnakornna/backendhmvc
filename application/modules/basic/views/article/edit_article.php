<?php 
	...
	$shear = $this->config->config['www']."/".$article_id;
...
?>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">												
												<div class="checkbox">
													<label>
														<input type="checkbox" class="ace" name="editor_picks" <?php if($article_list[0]['editor_picks_id'] > 0) echo "checked='checked'" ?> >
														<span class="lbl"> <?php echo $language['use']?></span>
													</label>
												</div>
										</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											...
											<code><i class="menu-icon fa fa-info"></i> Date Format MM/DD/YYY h:i:s <br> หากต้องการกำหนดวันเริ่มต้น และ สิ้นสุดการดูข้อมูลให้ใส่ <?php echo $language['display_date']?></code>
										</div>
									</div>
...
?>
								...
								<div class="form-group">
										...
										<div class="col-sm-9">
												<div id="fullToolbar">
													<textarea  id="embed_script" name="embed_script" style="width:100%;height:<?php echo ($embed_script != '') ? '340px' : '150px' ?>"><?php echo $embed_script?></textarea>
												</div>
												...
												if($relate_list)
														for($i=0;$i<count($relate_list);$i++){
															...
															if($relate_list[$i]->file_name != ''){
																$img_src = base_url('uploads/thumb300').'/'.$relate_list[$i]->folder.'/'.$relate_list[$i]->file_name;
																$display_img = (file_exists('uploads/thumb300/'.$relate_list[$i]->folder.'/'.$relate_list[$i]->file_name)) ? "<img src=".$img_src." height='50'>" : "";
...
<?php
		if($article_list[0]['approve'] == 1){
?>
										<a href="javascript:void(0);" class="twitter-share-button" data-url="<?php echo $shear ?>" data-text="<?php echo $shear_title?>" data-via="<?php echo $language['titleweb']?>" data-dnt="true"> <button type="button" class="btn btn-info share" >
											<i class="middle ace-icon fa fa-twitter-square white"> ทวีต</i>
										</button></a>	
<?php
		}
