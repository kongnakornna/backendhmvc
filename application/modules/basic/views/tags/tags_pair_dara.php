...
<?php
			...
			if($tags_list)
					for($i=0;$i<$alltags;$i++){
								...
								//$ref_type = $tags_list[$i]->ref_type;
								$ref_type = 'Dara Profile';
								...
								$url = base_url('dara/edit/'.$dara_profile_id);
?>
		<tr>
						<td>
								<?=$pair_id?>
						...
						<td><?=$ref_type?></td>
						<td><?=$tag_text?></td>
						<td><?=$dara_profile_id?></td>
						<td><a href="<?=$url?>" target=_blank><?=$dara_name?></a></td>

						<td class="hidden-480"><span class="col-sm-12">
								<label class="pull-right inline" id="enable<?=$tag_id?>">
								<small class="muted"></small>
									<input type="checkbox" name="status"  id="status<?=$tag_id?>" class="ace ace-switch ace-switch-4 status-buttons" data-value="<?php echo $tag_id?>" <?php if($tag_status  == 1) echo 'checked';?>  value=1>
