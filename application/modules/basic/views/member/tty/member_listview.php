...
<?php
			//Debug($member_list);
?>
											<table id="dataTables-member" class="table table-striped table-bordered table-hover">
												<thead>
													<tr>
														<th>No.</th>
														<th></th>
														...
														<th><?php echo $language['member_type'] ?></th>
														<th class="hidden-480">
...
?>
		<tr>
						...
						<td>
								<?=$number?>
								<input type="hidden" class="ace" name="selectid[]" value="<?=$member_profile_id?>" />
						</td>
						<td><?php if($picture != '') echo $picture ?></td>
						<td><a href="<?=$lnk_member?>"><?=$full_name?></a></td>
						<td><a href="<?=$lnk_member?>"><?=$nickname?></a></td>
