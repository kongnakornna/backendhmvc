...
.input-small{text-align: center;}
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<button class="btn btn-sm btn-primary" onclick="window.location='<?php echo site_url('subcategorymember/add/'.$cat) ?>';">
										<i class="ace-icon glyphicon glyphicon-plus align-top bigger-125"></i><?php echo $language['add_subcategory'].$language['member'] ?>
								</button>
...
<?php
	...
	echo form_open('subcategorymember/cat/'.$this->uri->segment(3), $attributes);
?>
		...
		<tr>
						...
						<td> 
															...
															<div class="hidden-sm hidden-xs action-buttons">
																...
																<a class="green" href="<?php echo site_url('subcategorymember/edit/'.$subcategory[$key]['subcategory_id_map'].'?category_id='.$this->uri->segment(3)); ?>">
															...
															<div class="hidden-md hidden-lg">
																<div class="inline position-relative">
																	...
																	<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																		...
																		<li>
																			<a href="<?php echo site_url('subcategorymember/add/'.$subcategory[$key]['subcategory_id_map'].'?category_id='.$this->uri->segment(3)); ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
																				<span class="green">
...
?>
		$('#status<?=$subcategory[$key]['subcategory_id_map']?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('subcategorymember/status/'.$subcategory[$key]['subcategory_id_map'])?>",
					//data: {id: res},
					cache: false,
		...
		$('#bootbox-confirm<?=$subcategory[$key]["subcategory_id_map"]?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								//alert('del');
								window.location='<?php echo base_url('subcategorymember/delete/'.$subcategory[$key]['subcategory_id_map'])?>';
						}
					});
		});
