...
<div class="col-xs-12">
				...
				<div class="col-xs-4">
									...
									</button> <?php echo $allnews." record." ?>
				</div>
<?php
													...
													<tr>
														...
														<th>
															...
															<?php if($category_id > 0){ ?>
																&nbsp;<i class="ace-icon fa fa-floppy-o bigger-120 blue" id="saveorder" data-rel="tooltip" title="Save Order" ></i>
															<?php } ?>
														</th>
														<th class="hidden-480">
...
?>
		<tr>
						...
						<td class="center">
									...
									<small class="muted"></small>
										<input type="checkbox" name="approve"  id="approve<?=$news_id2?>" class="ace ace-switch ace-switch-7" 
						...
						<td>
												...
												<div class="hidden-sm hidden-xs action-buttons">
																...
																<a class="red del-confirm" href="javascript:void(0);" id="bootbox-confirm<?=$news_id2?>" data-value="<? //$memberlist[$key]->_admin_id?>" data-name="<?=$title?>" data-rel="tooltip" title="Delete">
												...
												<div class="hidden-md hidden-lg">
																<div class="inline position-relative">
																	...
																	<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																		...
																		<li>
																			<a href="javascript:void(0);" id="bx-confirm<?=$news_id2?>" class="tooltip-error" data-rel="tooltip" title="Delete">
																				<span class="red">
...
?>
		/*$('#status<?=$news_id?>').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('news/status/'.$news_list[$i]['news_id2'])?>",
					//data: {id: res},
					cache: false,
					success: function(data){
							$("#msg_info").fadeOut();
		...
		$('#approve<?=$news_id?>').on('click', function() {
...
?>
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('news/approve/'.$news_list[$i]['news_id2'])?>",
					//data: {id: res},
					cache: false,
					success: function(data){
				...
				/*$.ajax({
					type: 'POST',
					url: "http://daraapi.siamsport.co.th/api/rest.php?method=DetailNews&key=5AckEziE&lang=th&news_id=<?php echo $news_list[$i]['news_id2']?>&gen_file=1",
					cache: false,
					success: function(data){
							$("#msg_success").attr('style','display:block;');
							AlertSuccess	('Generate json file.');
					}
				});*/
...
?>
		...
		$('#bootbox-confirm<?=$news_id?>').on('click', function() {
		...
		$('#bx-confirm<?=$news_id?>').on('click', function() {
				bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
					if(result) {
							//alert('del');
							window.location='<?php echo base_url('news/delete/'.$news_list[$i]['news_id2'])?>?cat=<?php echo $category_id ?>' ;
