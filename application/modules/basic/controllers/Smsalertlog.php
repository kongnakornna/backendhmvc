...
class Smsalertlog extends CI_Controller {
	...
	$enddate = $this->input->get_post('enddate',TRUE);
	   ...
	   // $pagination = $this->session->userdata($segment);
		...
		$pageConfig = doPagination($this->Smsalertlog_model->totalsmsalart($startdate,$enddate), $limit, $segment, site_url("/smsalertlog/listview/"));
		}
		//Debug($pageConfig);
		//die();
		...
		if($startdate!==''){
		$web_menu = $this->Smsalertlog_model->getsmsalertlog($pageIndex, $limit,$startdate,$enddate);
		}else{
		$web_menu = $this->Smsalertlog_model->getsmsalertlog($pageIndex, $limit);
		}
		
		//$web_menu = $this->Smsalertlog_model->getsmsalertlog($limit,$start * $limit);
		  //Debug($web_menu);
		  //die();
