...
class Report extends CI_Controller {
	...
	$enddate = $this->input->get_post('enddate',TRUE);
	   ...
	   // $pagination = $this->session->userdata($segment);
		...
		$breadcrumb[] = $language['report'];
		$webtitle = $language['report'].' - '.$language['titleweb'];
		...
		//$pageConfig = doPagination($this->Report_model->totalsensorlog($startdate,$enddate), $limit, $segment,$startdate,$enddate, site_url("/report/listview").$search_key);
		$pageConfig = doPagination($this->Report_model->totalsensorlog($startdate,$enddate), $limit, $segment,$startdate,$enddate, site_url("/report/listview"));
		//$pageConfig=$pageConfig;//.$search_key;
		}else{
		$pageConfig = doPagination($this->Report_model->totalsensorlog($startdate,$enddate), $limit, $segment, site_url("/report/listview/"));
		}
		//Debug($pageConfig);
		//die();
		...
		if($startdate!==''){
		$web_menu = $this->Report_model->getreport($pageIndex, $limit,$startdate,$enddate);
		}else{
		$web_menu = $this->Report_model->getreport($pageIndex, $limit);
		}
		
		//$web_menu = $this->Report_model->getreport($limit,$start * $limit);
		  //Debug($web_menu);
		  //die();
