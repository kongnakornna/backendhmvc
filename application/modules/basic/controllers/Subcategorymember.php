...
class Subcategorymember extends MY_Controller {

    public function __construct()    {
			parent::__construct();
			$this->load->model('SubCategory_Member_model');
			$breadcrumb = array();			
	...
	public function index(){
			...
			$breadcrumb[] = '<a href="'.base_url('subcategorymember').'">'.$language['category'].$language['member'].'</a>';
			$breadcrumb[] = $language['subcategory'].$language['member'];
			...
			$data = array(				
					"subcategory" => $this->SubCategory_Member_model->get_subcategory(),
					"content_view" => 'member/category/subcategory',
	...
	public function cat($id = 0){
			...
			if($id < 1){
				redirect(base_url('subcategorymember'));
			}
			$category_id	= 0;
			...
			$breadcrumb[] = '<a href="'.base_url('subcategorymember').'">'.$language['category'].'</a>';
			...
			if($this->input->server('REQUEST_METHOD') === 'POST'){
					...
					if(isset($search_form['selectid'])){
							...
							for($i=0;$i<$maxsel;$i++){
									...
									$this->SubCategory_Member_model->update_order($selectid[$i], $order[$i]);
									//Debug($this->db->last_query());

									if($tmp > $order[$i]){
											//Update ID ด้านหน้า
											//$this->SubCategory_Member_model->update_orderid_to_down($order[$i]);
									}
			...
			$data = array(
					...
					"subcategory" => $this->SubCategory_Member_model->get_subcategory($category_id),
					"content_view" => 'member/category/subcategory',
					"ListSelect" => $ListSelect,
	...
	public function add($catid = 0){
			...
			$breadcrumb[] = '<a href="'.base_url('subcategorymember').'">'.$language['category'].'</a>';
			$breadcrumb[] = '<a href="'.base_url('subcategorymember/cat/'.$catid).'">'.$language['subcategory'].'</a>';
			$breadcrumb[] = $language['add'];

			$data = array(
						"category_name" => $this->Category_model->get_category_by_id($catid),
						"content_view" => 'member/category/subcategory_add',
						"ListSelect" => $ListSelect,
						"breadcrumb" => $breadcrumb,
			);
			$this->load->view('template/template',$data);
	...
	public function edit($id = 0){
			...
			$cat_arr = $this->SubCategory_Member_model->get_subcategory_by_id($id);
			//Debug($this->db->lastquery());
			$data = array(
						"category_name" => $category_name,
						"cat_arr" => $cat_arr,
						"content_view" => 'member/category/subcategory_edit',
						"ListSelect" => $ListSelect,
						"breadcrumb" => $breadcrumb,
			);
			$this->load->view('template/template',$data);
	...
	public function list_dd($id = 0, $subcatid = 0){
			...
			$subcat = $this->SubCategory_Member_model->get_dd_list($id);
			//Debug($subcat);
	...
	public function status($id = 0){
		...
		if($id == 0){
			...
			$obj_status = $this->SubCategory_Member_model->get_status($id);
			...
			$data = array(
					"cat_arr" => $this->SubCategory_Member_model->status_subcategory($id, $cur_status),
			);
	...
	public function save(){
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST'){
				...
				$order_by = $this->SubCategory_Member_model->get_max_order($category_id);
				$get_max_id = $this->SubCategory_Member_model->get_max_id();
				...
				if ($this->form_validation->run()){
						...
						if($this->input->post('subcategory_id') > 0){ //UPDATE SQL
								...
								if($this->SubCategory_Member_model->update_subcategory($subcategory_id_en, $data_to_store)){
									$data['flash_message'] = TRUE; 
								}else{
									$data['flash_message'] = FALSE; 
								}
								if($this->SubCategory_Member_model->update_subcategory($subcategory_id_th, $data_to_store_th)){
									$data['flash_message'] = TRUE; 
								}else{
									$data['flash_message'] = FALSE; 
								}
								...
								//if the insert has returned true then we show the flash message
								if($this->SubCategory_Member_model->store_product($data_to_store)){
									$data['flash_message'] = TRUE; 
								}else{
									$data['flash_message'] = FALSE; 
								}

								if($this->SubCategory_Member_model->store_product($data_to_store_th)){
									$data['flash_message'] = TRUE; 
								}else{
									$data['flash_message'] = FALSE; 
								}
						...
						//$data['main_content'] = 'member/category/subcategory/add';						
						//"admin_menu" => $this->menufactory->getMenu(),
						$data = array(
							"content_view" => 'member/category/subcategory_add',
							"ListSelect" => $ListSelect,
        ...
        //load the view
			...
			if($this->form_validation->run()) redirect('subcategorymember/cat/'.$category_id);
	...
	public function delete($id){
			...
			$OBJnews = $this->SubCategory_Member_model->get_status($id);


			$subcategory_name = $OBJnews[0]['subcategory_name'];
			$order_by = $OBJnews[0]['order_by'];

			$this->SubCategory_Member_model->delete_subcategory($id);

			//**************Order New
			$this->SubCategory_Member_model->update_orderdel($order_by);
