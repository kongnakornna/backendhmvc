...
class Cremation extends CI_Controller {
    ...
    public function __construct()    {
			...
			$this->load->model('Cremation_model');
	...
	public function searchview(){
		...
		$province_id=$this->input->post('province_id_map');
		...
		$orderby='asc';
			...
			$membercountall = $this->Cremation_model->get_count();
			$object = json_decode(json_encode($membercountall), TRUE);
			#$membercount_all=$object[0]['countid']; 
			#$membercount_all='100';
		    $membercount_all=$listnum;
//pagination settings
        $config['per_page'] =$listnum;
        $config['base_url'] = base_url().'cremation/searchview';
		...
		/*
		echo '$countries_id='.$countries_id; echo'<br>';
		echo '$geography_id='.$geography_id; echo'<br>';
		echo '$province_id='.$province_id; echo'<br>';
		echo '$amphur_id='.$amphur_id; echo'<br>';
		echo '$district_id='.$district_id; echo'<br>';
		echo '$village_id='.$village_id; echo'<br>';
		echo '$sortby='.$sortby; echo'<br>';
		echo '$orderby='.$orderby; echo'<br>';
		echo '$searchkey='.$searchkey; echo'<br>';
		echo '$listnum='.$listnum; echo'<br>';
		exit();
		*/	
           ...
           ///////////////////////////////////////////////////////////////////////  
			$breadcrumb[] = $language['member'].$language['cremation'];
			$search_form = $this->input->post();
...
//////////////////////////////
				...
				$member_list = $this->Cremation_model->member_join(null, $keyword, $member_type, $gender, $sortby, 'Asc', $member_status, $startpage, $list_page);
				 
			}else{
				$keyword = "member";
				$member_list = $this->Cremation_model->member_join(null, null, null, '', 'lastupdate_date', 'asc', null, 0, $membercount_all);
			}
			
//start///$countries_id
					if(isset($search_form['countries_id'])){
							$countries_id = $search_form['countries_id'];
							$countries_list = $this->Countries_model->getSelectCountries($countries_id, 'countries_id', 'Member');
							//Debug($countries_list);
					}else{
							$countries_list = $this->Countries_model->getSelectCountries();
             				//Debug($countries_list);
            		}
//end ///$countries_id	
//start///$geography_id
...
//start///$memberstatus
					if(isset($search_form['status'])){
							...
							//$memberstatus = $this->Status_type_model->getSelectMemberstatus($status, 'status', 'Member');
...
//end ///$memberstatus	
			...
			$member_all=$this->Cremation_model->get_count();
			$data = array(
					...
					"content_view"=>'cremation/member_listview',
 ...
 $config['total_rows'] = $membercount_all;
 $config['per_page'] =$listnum; 
...
//////////////////////////////
				...
				$member_list = $this->Cremation_model->member_join(null, $keyword, $member_type, $gender, $sortby, 'Asc', $member_status, $startpage, $list_page);
				//$member_list = $this->Cremation_model->member_join(null, $keyword, $member_type, $gender, $sortby, 'Asc', $member_status, 0, 100);
			}else{
				$keyword = "member";
				$member_list = $this->Cremation_model->member_join(null, null, null, '', 'lastupdate_date', 'asc', null, 0, $membercount_all);
			}
	...
	public function search(){
			...
			if($this->input->server('REQUEST_METHOD') === 'GET'){
					...
					if(isset($search_form['kw'])){
						...
						$member_type_id = $member_id_map = $member_type_name = $first_name = $nick_name = "";

						$keyw = explode(" ", $search_form['kw']);
						$member_list = $this->Cremation_model->get_member_profile(null, $keyw, null, null, 'lastupdate_date', 'Asc', null, 0, 20);
						//$member_list = $this->Cremation_model->get_member_profile(null, $keyw);
						//Debug($member_list);
						if($member_list){
								...
								for($i=0;$i<$maxlist;$i++){
										...
										$url = $this->config->config['www'].'/member/'.$member_type_id.'/'.$member_id_map.'/'.($member_type_name).'/'.($nick_name.'-'.$first_name).'';
										//$url = "http://www.siammember.com/member/".$member_list[$i]['member_id_map'].".html";
										...
										$edit_data = base_url('member/edit/'.$member_id_map);
										//member/edit/1357
										...
										echo "<tr>
											<td>".$member_list[$i]['member_id_map']."</td>
											<td>".$tags_img."</td>
	...
	public function chkname(){
			...
			if($this->input->server('REQUEST_METHOD') === 'POST'){
					...
					if(isset($search_form['first_name'])){
							//echo "is keyword";
							//Debug($search_form);
							$member_list = $this->Cremation_model->search_member_profile('first_name', $search_form['first_name']);
							//Debug($member_list);
							$name = '';
					...
					if(isset($search_form['last_name'])){
							//echo "is keyword";
							//Debug($search_form);
							$member_list = $this->Cremation_model->search_member_profile('last_name', $search_form['last_name']);
							//Debug($member_list);
							$name = '';
					...
					if(isset($search_form['nick'])){
							//echo "is keyword";
							$member_list = $this->Cremation_model->search_member_profile('nick_name', $search_form['nick']);
							//Debug($member_list);
							$name = '';
							$maxlist = count($member_list);
							for($i=0;$i<$maxlist;$i++){
					...
					if(isset($search_form['pen_name'])){
							//echo "is keyword";
							$member_list = $this->Cremation_model->search_member_profile('pen_name', $search_form['pen_name']);
							//Debug($member_list);
							$name = '';
							$maxlist = count($member_list);
							for($i=0;$i<$maxlist;$i++){
...
////////////////////////////////////////
			...
			$ListSelect = $this->Api_model->user_menu($this->session->userdata('admin_type'));
			$language = $this->lang->language;
			$breadcrumb[] = '<a href="'.base_url('cremation').'">'.$language['cremation'].'</a>';
			...
			$data = array(
					...
					"village_list" => $village_list,
					"memberstatus" => $memberstatus,
					"content_view" => 'cremation/member_add',
					"breadcrumb" => $breadcrumb
			);
	...
	public function edit($id = 0){
			...
			//$member_type = $this->Member_type_model->get_member_type();
			$member_profile = $this->Cremation_model->get_member_profile($id);
			...
			$data = array(
					...
					"content_view" => 'cremation/member_edit',
					"breadcrumb" => $breadcrumb
			);
			//Ckeditor's configuration
			$data['ckeditor'] = array(
	...
	public function remove_img($member_id_map = 0){
			
			//echo $member_id_map ;
			$src = $this->input->post('name');
			//echo $member_id_map .' '. $src ;
			if(file_exists('uploads/member/'.$src)) unlink('uploads/member/'.$src);
			$obj_data['avatar'] = '';
			if($this->Cremation_model->store($member_id_map, $obj_data))
				echo 'Yes';
			else
				echo 'No';
	...
	public function picture($id = 0){
			...
			$data = array(
					...
					"member_id" => $id,
					"member_list" => $member_profile[0],
					"content_view" => 'cremation/member_edit_picture',
					"breadcrumb" => $breadcrumb
			);
			$this->load->view('template/template',$data);
 ...
 exit();
			if($data_input)
					...
					$new_data[$key] = $val;		
							//if($key != "editorCurrent" && $key != "tags" && $key != "tags_remove") 
							///if($key == "tags") $get_tags = explode(",", trim($val));
							//if($key == "tags_remove") $tags_remove = explode(",", $val);
							//if($key == "belong_to_id") $belong_to = explode(",", $val);
					}

			//if(!$new_data['member_id_map']) $new_data['member_id_map'] = $new_data['member_id_map'];
			if(!isset($new_data['member_id_map'])){
					$new_data['member_id_map'] = 0;
					//$new_data['member_id_map'] = 0;
			}
			...
			if(1 == 2){
					...
					if($data_input['birthday'] != '') $new_data['birthday'] = DateDB($data_input['birthday']);
					if($data_input['geography_id'] != '') $new_data['geography_id'] = DateDB($data_input['geography_id']);
					// Debug($new_data);
					// die();

					if($new_data['member_id_map'] > 0){
//Debug($new_data);
//die();
								$action = 2;
								$new_data['lastupdate_date'] = $now_date;
								$new_data['lastupdate_by'] = $this->session->userdata('admin_id');
								$this->Cremation_model->store($new_data['member_id_map'], $new_data);
								$data = array(
										"admin_menu" => $this->menufactory->getMenu(),
										"ListSelect" => $ListSelect,
										"member_type" => $this->Member_type_model->get_member_type(),
										"member_list" => $this->Cremation_model->get_member_profile(),
										"search_form" => null,
										"content_view" => 'cremation/member',
										"breadcrumb" => $breadcrumb,
										"success" => 'Update member Complete.'
								);
								$insert_newid = $new_data['member_id_map'];						
								
					}else{ //Insert DB
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								$action = 1;
								unset($new_data['member_id_map']);
								$now_date = date('Y-m-d H:i:s');
								$new_data['member_id1'] = '0';
								$new_data['status'] = '2';
								//$new_data['status2'] = $this->input->post('status');
								$new_data['diedate'] = '0000-00-00 00:00:00';
								$new_data['usedate'] = '0000-00-00';
								$new_data['balance'] = '450';
								$new_data['create_by'] = $this->session->userdata('admin_id');
								$new_data['update_by'] = $this->session->userdata('admin_id');
								$new_data['create_date'] = $now_date;
								$new_data['update_date'] = $now_date;
								$new_data['create_by'] = $this->session->userdata('admin_id');
								 //Debug($new_data);
								 //die();
								$insert_newid = $this->Cremation_model->store(0, $new_data);
								 //Debug($insert_newid);
								$data = array(
										"admin_menu" => $this->menufactory->getMenu(),
										"ListSelect" => $ListSelect,
					...
					$log_activity = array(
										...
										"ref_id" => $insert_newid,
										"ref_type" => 5,
										"ref_title" => trim($new_data['name']).' '.trim($new_data['first_name']." ".$new_data['last_name']),
										"action" => $action
					);			
...
/*
	public function status($id = 0){
			...
			if($id == 0){
					...
					$obj_status = $this->Cremation_model->get_status($id);
					...
					if($this->Cremation_model->store($id, $obj_data)) echo $cur_status;
					//$this->gen_json($id);
			}
	}
	*/
	public function delete($id){
			...
			$OBJMember = $this->Cremation_model->get_member_profile($id);
			$title = $OBJMember[0]['nick_name'].' '.$OBJMember[0]['first_name'].' '.$OBJMember[0]['last_name'];
			//$order_by = $OBJMember[0]['order_by'];

			$this->Cremation_model->delete_member_profile($id);
			//die();

			//$this->Cremation_model->update_orderdel($order_by);
	...
	public function delete_tags($txt){
			...
			$txt = urldecode($txt);
			//echo $txt;
			$result = $this->Cremation_model->delete_tag($txt, 5);
	...
	public function import_picture(){
			...
			$member_list = $this->Cremation_model->get_memberall();
			//Debug($member_list);
			//die();
	...
	public function import(){
			...
			$vdo_list = $this->Cremation_model->load_sstv(1, $number);
			...
			if(isset($vdo_list)){
				if($vdo_list['header']['code'] == 200){
					...
					if($insert == true)
						if($this->Cremation_model->import_sstv_to_db($data)){
	...
	public function auto_tags(){
			...
			$this->Cremation_model->auto_tags();
	}
