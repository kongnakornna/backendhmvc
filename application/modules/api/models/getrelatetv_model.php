...
class Getrelatetv_model extends CI_Model {
...
where am.status=1 ".$criticalCriteria."
	...
	public function get_cmsblogList($limit = 50, $offset = null, $order = null, $arrFilter = array()) 
	{
		...
		if($current_catList!=null && $current_catList!=""){
			...
			$current_zoneList = $filter['currentZoneIdList'];
		}else{
		...
		if (!$arrResult = $this->tppymemcached->get($cache_key)) {
			...
			if($query){
				...
				foreach($arrResult as $k => $v){
					...
					$filter['currentZoneIdList'] = $current_zoneList;
	...
	public function get_cmsblogCategoryCode($cmsblogID = 0, $categoryID = 0, $arrFilter = array()) 
	{
		...
		$cache_key = "cmsblogCategoryCode_".$cmsblogID."+".$categoryID;
		if (!$r = $this->tppymemcached->get($cache_key)) {
	...
	/////////////////
	function tvEpisode($id,$episode_id,$limit) {
		$data = array();
		$cache_key = "cache_tvEpisode_".$id."_".$episode_id."_".$limit;
		//$this->tppymemcached->delete($cache_key);
		if (!$data = $this->tppymemcached->get($cache_key)) {
			
			$this->load->model('connect_db_model', 'cdm');
			
			$sql_episode = "
			select e.*,p.psn_display_name,a.tv_name 
			from tv_program_episode e 
			left join tv_program a on e.tv_id=a.tv_id 
			left join personnel p on e.update_by=p.member_id 
			where e.tv_id='" . $id . "' and e.record_status=1 and e.tv_episode_id <> '" . $episode_id . "' 
			group by e.tv_episode_id order by e.tv_episode_onair DESC";
			$pagelen = 15;
			$sql_episode .= ' limit 0,' . $limit;
			$result_episode = $this->cdm->_query_row($sql_episode, 'select');
			# echo '<pre> result_episode=>'; print_r($result_episode); echo '</pre>'; Die();
			foreach ($result_episode as $k => $v) {
				...
				$data[] = array(
					   ...
					   // 'link' => base_url() . 'new/tv/tv_program_detail/' . $v['tv_id'] . '/' . $v['tv_episode_id'] . '/' . $this->trueplook->replace_char_url($tv_episode_name),
						...
						'linkapi' => base_url() . 'api/center/tvprogramdetail?tv_id=' . $v['tv_id'] . '&tv_episode_id=' . $v['tv_episode_id'],
						'view' => $this->trueplook->getViewCenter($v['tv_episode_id'], 'tv_program_episode', 'media'),
						'title' => $v['tv_name'],
						'desc' => $v['tv_episode_name'],
						'tv_episode_description' => $v['tv_episode_description'],				
						'add_date' =>  $v['add_date'],
						'last_update' =>  $v['last_update'],
						'update_by' =>  $v['update_by'],
						'tv_episode_tag' =>  $v['tv_episode_tag'],
						'cms_level_id' =>  $v['cms_level_id'],
						'cms_category_id' =>  $v['cms_category_id'],
						'tv_episode_onair' =>  $v['tv_episode_onair'],
						'content_stage' =>  $v['content_stage'],
						'view_count' =>  $v['view_count'],
						'psn_display_name' => $v['psn_display_name'],
						'tv_name' =>  $v['tv_name']	
				);
			}
			$this->tppymemcached->set($cache_key, $data,600);
		}
        return $data;
    }
	function tvEpisodeDetail($id,$episode_id,$limit) {
		$data = array();
		$cache_key = "cache_tvEpisode_".$id."_".$episode_id."_".$limit;
		//$this->tppymemcached->delete($cache_key);
		if (!$data = $this->tppymemcached->get($cache_key)) {
