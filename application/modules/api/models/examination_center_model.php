...
$rs=$this->tppymemcached->get($cache_key_encrypt);
*/
...
,concat('http://www.trueplookpanya.com/api/knowledgebase/content/',content.mul_content_id) AS content_api_url from mul_map_content_lesson map 
...
FROM 
  ...
  LEFT JOIN (
    (
      SELECT 
        hashtag_map.project_id, 
        hashtag_map.content_id, 
        CONCAT(
          'http://www.trueplookpanya.com/tcas/quotanews?id=', 
          hashtag_map.content_id
        ) as 'content_url', 
        ams_news_directapply.news_title as 'content_title', 
        ams_news_directapply.news_detail as 'content_detail', 
        CONCAT(
          'http://static.trueplookpanya.com/', 
          REPLACE(
            ams_news_directapply.news_banner_file, 
            '\\', '/'
          )
        ) as 'content_image', 
        'ข่าวรับตรง' as 'content_type', 
        'http://www.trueplookpanya.com/tcas/quotanews' as 'content_type_url', 
        ams_news_directapply.news_view as 'viewcount' 
      FROM 
        hashtag_map 
        LEFT JOIN ams_news_directapply ON hashtag_map.content_id = ams_news_directapply.news_id 
      WHERE 
        hashtag_map.project_id = 3 
        AND ams_news_directapply.record_status = 1
    ) 
    UNION 
      (
        SELECT 
          ...
          hashtag_map.content_id, 
          CONCAT(
            'http://www.trueplookpanya.com/tcas/campnews/detail/', 
            hashtag_map.content_id
          ) as 'content_url', 
          ams_news_camp.camp_title as 'content_title', 
          ams_news_camp.camp_detail as 'content_detail', 
          CONCAT(
            'http://static.trueplookpanya.com/', 
            REPLACE(ams_news_camp.banner, '\\', '/')
          ) as 'content_image', 
          'ข่าวค่าย' as 'content_type', 
          'http://www.trueplookpanya.com/tcas/campnews' as 'content_type_url', 
          ams_news_camp.viewcount as 'viewcount' 
        FROM 
          hashtag_map 
          LEFT JOIN ams_news_camp ON hashtag_map.content_id = ams_news_camp.camp_id 
        WHERE 
          hashtag_map.project_id = 4 
          AND ams_news_camp.isEnable = 1
      ) 
    UNION 
      (
        SELECT 
          ...
          hashtag_map.content_id, 
          CONCAT(
            'http://www.trueplookpanya.com/knowledge/content/', 
            hashtag_map.content_id
          ) as 'content_url', 
          cmsblog_detail.title as 'content_title', 
          cmsblog_detail.description_short as 'content_detail', 
          CONCAT(
            'http://static.trueplookpanya.com/', 
            REPLACE(
              cmsblog_detail.thumb_path, '\\', 
              '/'
            )
          ) as 'content_image', 
          'Blog' as 'content_type', 
          'http://www.trueplookpanya.com/knowledge/content' as 'content_type_url', 
          cmsblog_detail.view_count as 'viewcount' 
        FROM 
          hashtag_map 
          LEFT JOIN cmsblog_detail ON hashtag_map.content_id = cmsblog_detail.idx 
        WHERE 
          hashtag_map.project_id IN (5, 8, 9, 10, 11, 12, 13) 
          AND cmsblog_detail.record_status >= 0
      ) 
    UNION 
      (
        SELECT 
          ...
          hashtag_map.content_id, 
          CONCAT(
            'http://www.trueplookpanya.com/learning/detail/', 
            hashtag_map.content_id
          ) as 'content_url', 
          mul_content.mul_content_subject as 'content_title', 
          mul_content.mul_content_text as 'content_detail', 
          NULL as 'content_image', 
          'คลังบทเรียน' as 'content_type', 
          'http://www.trueplookpanya.com/learning' as 'content_type_url', 
          mul_content.view_count as 'viewcount' 
        FROM 
          hashtag_map 
          LEFT JOIN mul_content ON hashtag_map.content_id = mul_content.mul_content_id 
        WHERE 
          hashtag_map.project_id = 6 
          AND mul_content.mul_content_status = 1
      ) 
    UNION 
      (
        SELECT 
          ...
          hashtag_map.content_id, 
          CONCAT(
            'http://www.trueplookpanya.com/examination/doexam/', 
            hashtag_map.content_id
          ) as 'content_url', 
          cvs_course_examination.exam_name as 'content_title', 
          NULL as 'content_detail', 
          NULL as 'content_image', 
          'คลังข้อสอบ' as 'content_type', 
          'http://www.trueplookpanya.com/examination' as 'content_type_url', 
          NULL as 'viewcount' 
        FROM 
          hashtag_map 
          LEFT JOIN cvs_course_examination ON hashtag_map.content_id = cvs_course_examination.id 
        WHERE 
          hashtag_map.project_id = 7 
          AND cvs_course_examination.exam_status = 'yes'
      )
  ) AS tbl2 ON tbl1.content_id = tbl2.content_id 
  LEFT JOIN (
    SELECT 
      hashtag_map.content_id, 
      GROUP_CONCAT(hashtag.tag_name) hashtag 
    FROM 
      hashtag_map 
      LEFT JOIN hashtag on hashtag_map.hashtag_id = hashtag.idx 
    GROUP BY 
      hashtag_map.content_id
  ) as tbl3 ON tbl1.content_id = tbl3.content_id 
WHERE 
  tbl1.hashtag_id IN (
    SELECT 
      idx 
  ...
  AND tbl2.content_id IS NOT NULL 
GROUP BY 
  tbl1.content_id
";
//echo '<pre>   $sql=>'; print_r($sql); echo '</pre>';  Die();
