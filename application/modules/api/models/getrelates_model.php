...
class getRelates_model extends CI_Model {
...
where am.status=1 ".$criticalCriteria."
	...
	public function getDetail_cmsblog($cmsblogID, $arrFilter = array())
	{
			...
			$sql = "select 
					...
					,CONCAT('".$this->config->item('static_url')."', COALESCE(cms.thumb_path, 'no-image.png')) thumbnail
	...
	public function get_cmsblogList($limit = 50, $offset = null, $order = null, $arrFilter = array()) 
	{
		...
		$sql = "select 
					...
					,cms.title as topic
					,cms.description_short as short_detail
					,cms.description_long as description
					,concat('http://www.trueplookpanya.com/knowledge/content/',cms.idx) as web_url
					,CONCAT('".$this->config->item('static_url')."', COALESCE(cms.thumb_path, 'no-image.png')) thumbnail
					,cms.view_count as viewcount
					,cms.start_date as addDateTime
	...
	///////////////////
	public function get_cmsblogListv2($limit = 50, $offset = null, $order = null, $arrFilter = array()){
		$DBSelect = $this->load->database('select', TRUE);
		...
		$sql = "select 
					...
					,cms.title as topic
					,cms.description_short as short_detail
					,concat('".$this->config->item('base_url')."knowledge/content/',cms.idx) as web_url
					,CONCAT('".$this->config->item('static_url')."', COALESCE(cms.thumb_path, 'no-image.png')) thumbnail
					,cms.view_count as viewcount
					,cms.start_date as addDateTime
					,cms.event_date as event_date
					...
					,CONCAT('".$this->config->item('static_url')."', COALESCE(cms.thumb_path, 'cmsblog/default_thumbnail.png')) thumb_path
					,CONCAT('".$this->config->item('static_url')."', COALESCE(cms.banner_path, 'cmsblog/default_thumbnail.png')) banner_path
