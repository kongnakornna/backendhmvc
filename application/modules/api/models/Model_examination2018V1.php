...
$sql.="select distinct dc.*,ces.exam_status public_status
...
,stat.content_id,stat.view as views
from cvs_course_examination as dc 
inner join cvs_course_exam_share as ces on ces.exam_id=dc.id
inner join cvs_stat_all as stat on stat.content_id=dc.id 
where dc.exam_status='yes' and stat.category_id=2";
...
from cvs_course_examination as dc 
inner join cvs_course_exam_share as ces on ces.exam_id=dc.id
inner join cvs_stat_all as stat on stat.content_id like dc.id
where dc.exam_status='yes' and stat.category_id=2 and ces.exam_status='shared'
";
if($searchtype=="member_id"){
$sql .=" and dc.member_id LIKE %$keyword% "; 
}else if($searchtype=="exam_code"){
...
###########DB SQL query Start###########
$start=($page-1)*$perpage;
$sql.="select distinct stat.content_id,dc.*
,ces.exam_status as public_status
,(select count(id) from cvs_course_question cc where cc.exam_id=dc.id) question_count
,(select mul_level_name from mul_level where mul_level_id=dc.mul_level_id) level_name
,(select mul_category_name from mul_category_2014 where mul_category_id=dc.mul_root_id) subject_name
,dc.id,stat.view as views
from cvs_course_examination as dc 
inner join cvs_course_exam_share as ces on ces.exam_id=dc.id
inner join cvs_stat_all as stat on stat.content_id like dc.id
where dc.exam_status='yes' and stat.category_id=2 and ces.exam_status='shared'";
...
$sql.=" order by dc.exam_add_date desc,stat.view desc limit $start,$perpage";
$query=$this->db->query($sql);
$resultsdata=$query->result();
$row=$query->num_rows(); 	
###########DB SQL query End ###########
...
public function get_searchall_root_id($keyword,$level_id,$category_id,$perpage,$page,$searchtype,$deletekey){    
if($perpage==''){$perpage=100;}
####################################
##########*******memcache*******############
$search=',';
...
if($orderby=='asc'){ 
$orderby='asc';
}elseif($orderby=='dsc' || $orderby=='desc'){ 
$orderby='desc';
}else{ 
$orderby='desc';
}
...
";
if($log_id==null){
$sql.="where logscore.exam_id=$exam_id and logscore.user_id=$user_id order by logscore.id $orderby ";
if($limit!=''){$sql.=" limit $limit ";}   
}elseif($log_id!==null){

$sql.="where logscore.exam_id=$exam_id and logscore.user_id=$user_id and logscore.id=$log_id order by logscore.id $orderby "; 
$sql.=" limit 1";
}
...
$cachetime='600';
if($log_id==null){
$cachekey="key-where-cvs-course-exam".$exam_id."-score-user_id-".$user_id.'-orderby-'.$orderby;
}elseif($log_id!==null){
$cachekey="key-where-cvs-course-exam".$exam_id."-score-user_id-".$user_id.'-log_id-'.$log_id.'-orderby-'.$orderby;       
}
#################################### 
#echo '<pre>$sql=>';print_r($sql);echo '</pre>';  Die();
#################################### 
