<?php
class Examination_test_model extends CI_Model{
...
public function getListall($exam_id='',$limit=100,$exam_name='',$delete_cache_key=0) {
if($exam_id!==Null){
...
public function getcountall($exam_name='',$delete_cache_key=0) {
if($exam_name!==Null){
...
public function getcourse_examscore($exam_id='',$member_id='',$delete_cache_key=0) {
...
if($result_data=='yes'){$result_data='yes';
}else{$result_data='no';}
return $result_data;    
}   
...
$result_data=$this->db->where('exam_id',$id);
$result_data=$this->db->update('cvs_course_examination',$data);  
if($result_data=='yes'){$result_data='no';}else{$result_data='yes';}
return $result_data;    
}	
...
$delete_cache_key=@$this->input->get('offset');
if($delete_cache_key==Null){$delete_cache_key=0;}
...
public function get_examination_user_log($delete_cache_key=0) {
...
if($delete_cache_key==Null){ 
$delete_cache_key=@$this->input->get('delete_cache_key');
 }
#########*****cache*****#############
...
public function get_question_user_log($exam_id,$member_id,$delete_cache_key=0){
...
public function get_course_exam_scorelistalllog($delete_cache_key=0){
$member_id=@$this->input->get('member_id');
$user_id=@$this->input->get('user_id');
if($user_id==Null){
...
public function get_examinationgraphbycourse_row($cache_key=0){
$exam_id=@$this->input->get('exam_id');
...
public function get_examinationgraphbycourse_scoreallanswer($exam_id='',$startdate='',$enddate='',$delete_cache_key=0){
$exam_id=@$this->input->get('exam_id');
$startdate=gmdate("Y-m-d", $startdate);
$enddate=gmdate("Y-m-d", $enddate);
...
public function get_examinationgraphbycourse_count($cache_key=0){
$exam_id=@$this->input->get('exam_id');
$sql="SELECT count(*) as total from cvs_course_question where exam_id=$exam_id";
#echo 'sql=>'.$sql; Die();
#########*****cache*****#############
$cache_key="get_examinationgraphbycourse_count_".$exam_id.'_'.$sql;
//$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
###2017-10
public function get_lesson($limit=50,$id='',$delete_cache_key=0){
if($id==Null){
...
public function where_mul_map_level_lesson($id,$type,$delete_cache_key=0,$delete_cache_key=0){
if($type==1){
...
#####################**2017-10-03**#####################
public function where_map_examquestionlesson($id,$type,$delete_cache_key=0){
if($type==1){
$sql="select * 
...
public function where_course_examination($id,$delete_cache_key=0){
$sql="select * 
...
$data=$rsdata->result_array();
$numrows=$this->db->query($sql)->num_rows();
$datas = array('sql'=>$sql,'rs'=>$data,'numrows'=>$numrows,);
################
$this->tppymemcached->set($cache_key,$data,3600);
}
/*
...
public function where_question($id,$delete_cache_key=0){
...
public function where_question_id($id,$delete_cache_key=0){
$sql="select distinct q.*
,(select exam_name from  cvs_course_examination where id=q.exam_id)as exam_name
...
public function where_map_choice($id,$delete_cache_key=0){
...
public function where_map_choice_all($id,$delete_cache_key=0){
...
public function where_map_choice_true($id,$delete_cache_key=0){
$sql="select question_id as id_question,ans.id as id_ans,ans.answer_detail as detail_answer,ans.answer_ans as status_answer,answer_comment
,(select question_detail from  cvs_course_question where id=ans.question_id)as detail_question
...
public function where_mul_lesson_id($id,$delete_cache_key=0){
$sql="SELECT *
...
public function where_examination_id($id,$delete_cache_key=0){
...
#############
public function where_count_lesson_mul_map_exam_question_lesson_exam_id1($id,$delete_cache_key=0){
#$sql="select ls.lesson_id,ls.lesson_name  
...
public function where_count_lesson_mul_map_exam_question_lesson_exam_id($id,$delete_cache_key=0){
#$sql="select ls.lesson_id,ls.lesson_name  
...
public function where_mul_map_content_lessonid($lesson_id,$delete_cache_key=0){
...
public function where_mul_map_exam_question_lesson_content($lesson_id,$delete_cache_key=0){
#echo '<pre> lesson_id=>'; print_r($lesson_id); echo '</pre>';
...
public function where_map_examquestion_examlessontrue($exam_id,$lesson_id,$delete_cache_key=0){
#echo '<pre> lesson_id=>'; print_r($lesson_id); echo '</pre>';
...
public function where_mul_map_exam_question_lesson_id($lesson_id,$delete_cache_key=0){
#echo '<pre> lesson_id=>'; print_r($lesson_id); echo '</pre>';
...
public function where_mul_map_exam_question_lesson_id_exam_id($lesson_id,$exam_id,$delete_cache_key=0){
$sql="select * from mul_map_exam_question_lesson as map  
left join cvs_course_question as q on q.id=map.question_id
...
#############
public function scoredisplay_exam_score($exam_id,$delete_cache_key=0){
		$sql = "SELECT COUNT(*) cnt FROM cvs_course_question WHERE exam_id=$exam_id";
		$cache_key = "QuestionCNT_ScoreDisplay_exam_score_member_".$sql;
		//$this->tppymemcached->delete($cache_key);
  if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
		if (!$question_total = $this->tppymemcached->get($cache_key)) {
			$question_total=$this->db->query($sql)->row()->cnt;
			$this->tppymemcached->set($cache_key, $query,300);
		}
...
$cache_key = "ScoreValue_ScoreDisplay_exam_score_member_".$sql2;
		//$this->tppymemcached->delete($cache_key);
		if (!$scoreAllAnswer = $this->tppymemcached->get($cache_key)) {
			$scoreAllAnswer = $this->db->query($sql2)->result_array();
			$this->tppymemcached->set($cache_key, $query,60);
		}
...
public function scoredisplay_exam_score_school($exam_id,$schoolname='',$level='',$delete_cache_key=0){
		$sql = "SELECT COUNT(*) cnt FROM cvs_course_question WHERE exam_id=$exam_id";
...
public function scoredisplay_exam_score_and_course_exam_score_member($exam_id,$job_edu_name,$job_edu_level,$delete_cache_key=0){
		$sql = "SELECT COUNT(*) cnt FROM cvs_course_question WHERE exam_id=$exam_id";
...
public function where_cvs_course_exam_score_member($exam_id,$member_id='',$order='desc',$limit='10000',$delete_cache_key=0){
 if($member_id==Null){
...
from cvs_course_exam_score as score  where score.exam_id='$exam_id'  order by score.id asc limit $limit";
}else{
$sql="select distinct *,(select count( score_value )from cvs_course_exam_score where exam_id=score.exam_id)as total,(select count(*) from cvs_course_question where exam_id=score.exam_id)as cnt
...
if($order=='asc'){
$sql="select distinct *,(select count( score_value )from cvs_course_exam_score where exam_id=score.exam_id)as total,(select count(*) from cvs_course_question where exam_id=score.exam_id)as cnt
from cvs_course_exam_score as score  where score.exam_id='$exam_id' and score.member_id='$member_id' order by score.id asc limit $limit";
}else{
$sql="select distinct *,(select count( score_value )from cvs_course_exam_score where exam_id=score.exam_id)as total,(select count(*) from cvs_course_question where exam_id=score.exam_id)as cnt
...
public function where_cvs_course_exam_score_member_user_id($exam_id,$user_id,$order='desc',$limit='10000',$delete_cache_key=0){
$sql="select distinct *
...
public function where_cvs_course_exam_score_member_limit($exam_id,$member_id,$limit,$delete_cache_key=0){
 if($member_id==Null){
  //$sql="SELECT distinct * from cvs_course_exam_score as score  where score.exam_id='$exam_id' order by score.id desc";
$sql="select distinct *,(select count( score_value )from cvs_course_exam_score where exam_id=score.exam_id)as total,(select count(*) from cvs_course_question where exam_id=score.exam_id)as cnt
from cvs_course_exam_score as score  where score.exam_id='$exam_id'  order by score.id desc limit $limit";
...
public function where_cvs_course_exam_answer($question_id,$id,$delete_cache_key=0){
...
public function where_cvs_course_exam_score($exam_id,$member_id,$delete_cache_key=0){
...
##########################################################
public function where_cvs_course_exam_answer_all_true($question_id,$delete_cache_key=0){
...
public function where_cvs_course_exam_answer_all_user($question_id,$id,$delete_cache_key=0){
$sql="SELECT distinct * from cvs_course_answer  where question_id='$question_id' and id='$id' order by  id desc ";
$cache_key = "qaExamination_where_cvs_course_exam_answer_all_".$sql;
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$this->tppymemcached->delete($cache_key);
...
################### 
/*
$sql1="SELECT *, 'upload' as folder_path FROM users_account u LEFT OUTER JOIN blog ON blog.member_id = u.member_id WHERE user_id=$user_id";
//echo '<pre> sql1=>'; print_r($sql1); echo '</pre>'; Die();  
...
public function where_mul_map_content_lessonid_join($lesson_id,$delete_cache_key=0){
...
############
public function where_user_by_school($job_edu_name,$delete_cache_key=0){
...
public function where_user_by_school_level($job_edu_name,$job_edu_level,$delete_cache_key=0){
...
public function where_exam_id_job_edu_name_user_by_school_between_($exam_id,$job_edu_name,$datestart,$enddate,$delete_cache_key=0){
...
public function where_exam_id_job_edu_job_edu_level_name_user_by_school_between_($exam_id,$job_edu_name,$job_edu_level,$datestart,$enddate,$delete_cache_key=0){
$sql="select distinct users.user_id,users.member_id,psn_firstname as firstname,users.psn_lastname as lastname
,case users.psn_sex when '1' then 'ชาย' when '2' then 'หญิง' when '' then 'ไม่ระบุเพศ' end as sex
,users.psn_address as address,users.psn_address as address
,users.psn_postcode,users.psn_province as province,users.psn_id_number as id_number,users.psn_birthdate as birthdate
...
############
public function examinationlistwhere_array2s($array,$delete_cache_key=0){
...
public function examinationlistwhere_array2($array,$delete_cache_key=0){
$sql="SELECT * from cvs_course_examination where id in ($array)  order by FIND_IN_SET (id,'$array')";
...
public function examinationlistwhere_array($array,$delete_cache_key=0){
#$sql="SELECT * from cvs_course_examination where id in ($array) and exam_status='yes' order by id asc";
 #$sql="SELECT * from cvs_course_examination where id in ('$array')  order by FIND_IN_SET (id,'$array')";
 #$sql="SELECT * from cvs_course_examination where id in ($array)  order by id asc";
 $sql="SELECT * from cvs_course_examination where id in ($array) order by FIND_IN_SET (id,'$array')";
...
public function examination_distinct_edu_name_edu_level_exam_id($job_edu_name,$job_edu_level,$exam_id,$delete_cache_key=0){
$sql="select distinct users.user_id,users.member_id,psn_firstname as firstname,users.psn_lastname as lastname
...
public function examination_distinct_edu_name_edu_level_exam_id2($exam_id,$delete_cache_key=0){
$sql="select distinct users.user_id,users.member_id,psn_firstname as firstname,users.psn_lastname as lastname
,COALESCE(NULLIF(concat(users.psn_firstname,' ',users.psn_lastname),''),users.psn_display_name) as user_fullname
,case users.psn_sex when '1' then 'ชาย' when '2' then 'หญิง' when '' then 'ไม่ระบุเพศ' end as sex
,users.psn_address as address,users.psn_address as address
...
where score.id>24000000  and score.exam_id=$exam_id order by users.user_id asc,score.id desc ";
 ...
 #echo '<pre> $sql=>'; print_r($sql); echo '</pre>';  Die();
$cache_key = "where_examination_distinct_edu_name_edu_level_exam_id2_".$sql;
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
#$this->tppymemcached->delete($cache_key);
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
public function examination_distinct_edu_name_exam_id($job_edu_name,$exam_id,$delete_cache_key=0){
$sql="
...
public function examination_maxscore_score_users_exam_id($exam_id,$id,$type='',$delete_cache_key=0){
if($type=='' || $type==1){
...
public function examination_maxid_score_users_exam_id($exam_id,$id,$type='',$delete_cache_key=0){
if($type=='' || $type==1){
...
public function examination_maxid_score_question_total_users_exam_id($exam_id,$id,$type='',$delete_cache_key=0){
if($type=='' || $type==1){
 $sql="select distinct users.user_id,users.member_id,psn_firstname as firstname,users.psn_lastname as lastname
,case users.psn_sex when '1' then 'ชาย' when '2' then 'หญิง' when '' then 'ไม่ระบุเพศ' end as sex
,users.psn_address as address,users.psn_address as address
...
public function count_user_edu_name_level($job_edu_name,$job_edu_level,$delete_cache_key=0){
...
public function count_user_edu_name($job_edu_name,$delete_cache_key=0){
...
public function mul_lesson_id_exam_id_question_id($lesson_id,$exam_id,$question_id,$delete_cache_key=0){
...
#########
public function cvs_course_question_id($exam_id,$delete_cache_key=0){
...
public function cvs_course_answer_qid_type($question_id,$type=1,$delete_cache_key=0){
 if($type==1){
...
public function total_answer_true($result_set,$delete_cache_key=0){
...
####chart
public function chart_score_value($exam_id,$delete_cache_key=0){
...
public function chart_score_value_member_id($exam_id,$member_id,$delete_cache_key=0){
$sql="SELECT score.exam_id,score.score_value,count(score.score_value) as total 
FROM cvs_course_exam_score  as score
left join cvs_course_question as exam on exam.id=score.exam_id
...
public function chart_score_ansall($exam_id,$delete_cache_key=0){
...
public function chart_score_question_id($exam_id,$delete_cache_key=0){
...
##########
public function chart_lesson_name_exam_id($exam_id,$delete_cache_key=0){
$sql="select distinct map_eql.lesson_id 
...
public function where_in_cvs_course_answer_true($coursequestionid,$delete_cache_key=0){
...
##########
public function where_in_question_lesson_id_ans_true($coursequestionid,$lesson_id,$delete_cache_key=0){
 /*
$sql="select *
...
##########
public function job_edu_name_users_account($job_edu_name,$delete_cache_key=0){
 /*
...
public function job_edu_name_users_account_like($job_edu_name,$delete_cache_key=0){
...
public function job_edu_name_users_account_countdata_row($exam_id,$job_edu_name,$job_edu_level,$delete_cache_key=0){
...
public function where_cvs_course_answer($id,$delete_cache_key=0){
$sql="select ans.id as ans_id,ans.answer_detail as detail,ans.answer_ans as answer
,(select question_detail from  cvs_course_question where id=ans.question_id)as question_detail
from cvs_course_answer as ans where question_id='$id' order by id asc";
...
public function getSelectdata2_exam_id($array='13462',$name="exam_id",$default){	
   $this->load->helper('common');
   ...
   if($array!==Null){
    $rows=$this->get_category_by_parent_id($array);
     ...
     $rows=$this->get_category_by_parent_id($array='13462');
   }
	    	$opt = array();
...
public function get_category_by_parent_id($array,$delete_cache_key=0){
#$sql = "SELECT * from cvs_course_examination where id in ($array) and exam_status='yes' order by id asc";
#$sql = "SELECT * from cvs_course_examination where id in ($array)  order by id asc";
$sql = "SELECT * from cvs_course_examination where id in ($array) ORDER BY FIND_IN_SET(id, '".$array."')";
...
public function avg_school_level_exam_id($job_edu_name,$job_edu_level,$exam_id,$delete_cache_key=0){
...
##########  school  name
public function avg_school_level_exam_id2($exam_id,$delete_cache_key=0){
$sql="select avg(score.score_value) as score_avg
,(select count(id) from cvs_course_question where exam_id=exam.id)as question_total
from  users_account as users   
left join cvs_course_exam_score score on score.user_id=users.user_id
left join cvs_course_examination exam on exam.id=score.exam_id
where score.id>24000000 and  score.exam_id=$exam_id order by users.user_id asc,score.id desc ";
$cache_key = "avg_school_level_exam_id2'.$sql.";
//$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
##########
public function reportbyschoolall_job_edu($exam_id,$job_edu_name,$delete_cache_key=0){
$sql="select distinct users.member_id
from cvs_course_exam_score  as score
left join cvs_course_examination as exam on exam.id=score.exam_id
left join users_account as users on users.member_id=score.member_id
where score.exam_id=$exam_id and users.job_edu_name='$job_edu_name'";
...
public function reportbyschoolall_job_edu2($exam_id,$delete_cache_key=0){
$sql="select distinct users.member_id
from cvs_course_exam_score  as score
left join cvs_course_examination as exam on exam.id=score.exam_id
left join users_account as users on users.member_id=score.member_id
where score.exam_id=$exam_id ";
$cache_key = "reportbyschoolalls_job_edu2_".$sql;
#$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
public function schoolscoredisplay_exam_score_and_course_exam_score_member($exam_id,$job_edu_name,$delete_cache_key=0){
		$sql = "SELECT COUNT(*) cnt FROM cvs_course_question WHERE exam_id=$exam_id";
...
public function school_examination_distinct_edu_name_edu_level_exam_id($job_edu_name,$exam_id,$delete_cache_key=0){
...
public function school_examination_distinct_edu_name_edu_level_exam_id2($exam_id,$delete_cache_key=0){
...
left join cvs_course_examination exam on exam.id=score.exam_id
where score.id>24000000  and score.exam_id=$exam_id order by users.user_id asc,score.id desc ";
$cache_key = "schools_where_examination_distinct_edu_name_edu_level_exam_id2_".$sql;
#$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
public function school_count_user_all($job_edu_name,$delete_cache_key=0){
...
public function avg_school_level_exam_all($job_edu_name,$exam_id,$delete_cache_key=0){
$sql="select avg(score.score_value) as score_avg
,(select count(id) from cvs_course_question where exam_id=exam.id)as question_total
from  users_account as users   
left join cvs_course_exam_score score on score.user_id=users.user_id
...
##########
public function school_count_user_all2($exam_id,$delete_cache_key=0){
$sql="SELECT score.score_value, count(score.score_value ) as total FROM cvs_course_exam_score as  score left join cvs_course_examination as exam on exam.id=score.exam_id 
left join users_account as users on users.member_id=score.member_id
WHERE score.exam_id=$exam_id 
GROUP BY score.score_value  order by  score.score_value desc ";
$cache_key = "school_count_user_alls2_".$sql;
#$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
public function avg_school_level_exam_all2($exam_id,$delete_cache_key=0){
$sql="select avg(score.score_value) as score_avg
,(select count(id) from cvs_course_question where exam_id=exam.id)as question_total
from  users_account as users   
left join cvs_course_exam_score score on score.user_id=users.user_id
left join cvs_course_examination exam on exam.id=score.exam_id
where score.id>24000000 and score.exam_id=$exam_id order by users.user_id asc,score.id desc";
$cache_key = "avg_school_level_exam_all2'.$sql.";
//$this->tppymemcached->delete($cache_key);
if($delete_cache_key==1){ $this->tppymemcached->delete($cache_key); }
$data=$this->tppymemcached->get($cache_key);
if(!$data){
...
##########2017-12-19
public function count_user_edu_name_levelv2($job_edu_name,$job_edu_level,$delete_cache_key=0){
$sql="select count(user_id)as count_user from  users_account as users where users.job_edu_name='$job_edu_name' and users.job_edu_level='$job_edu_level'";
#echo '<pre> $sql=>'; print_r($sql); echo '</pre>';  Die();
$cache_key = "count_user_edu_name_level_data_".$sql;
//$this->tppymemcached->delete($cache_key);
