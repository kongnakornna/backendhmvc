...
$string=$idin;
$schoolset=str_replace($search,$replace,$string);
$cachekey='key-count-cvscourseexamination-keyword-'.$keyword.'-level_id-'.$level_id.'-category_id-'.$category_id.'-searchtype-'.$searchtype;
##############
//Load library
$this->load->library('Memcached_library');
#$cachetime='300';
...
if(!$resultsdata){
###########DB SQL query Start###########
$sql.="select count(dc.id) as coun_rows from cvs_course_examination dc
left join cvs_course_exam_share ces on ces.exam_id=dc.id 
where  dc.exam_status='yes'
";
...
public function where_cvs_course_exam_score_user_id($exam_id,$user_id,$deletekey){
...
where logscore.exam_id=$exam_id and logscore.user_id=$user_id";
$this->load->library('Memcached_library');
$cachetime='600';
$cachekey="key-where-cvs-course-exam".$exam_id."-score-user_id-".$user_id;
#################################### 
#echo '<pre>$sql=>';print_r($sql);echo '</pre>';  Die();
#################################### 
##########*******memcache*******############
