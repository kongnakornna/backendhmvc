...
public function get_status($id,$lang=''){
	if($lang==null){ $language = $this->lang->language['lang']; }else{ $language = $lang; 	}
...
public function get_max_order($province_id = 0,$lang=''){
	if($lang==null){ $language = $this->lang->language['lang']; }else{ $language = $lang; 	}
...
public function getSelectProvince($province_id = 0, $default = 0,$name = "province_id",$lang=''){
	if($lang==null){ $language = $this->lang->language['lang']; }else{ $language = $lang; 	}
    	//debug($language);
...
public function province($province_id=0, $order='order_by', $order_type='Asc', $limit_start = 0, $listpage = 2000000000,$lang=''){
	if($lang==null){ $language = $this->lang->language['lang']; }else{ $language = $lang; 	}  
		$this->db->select('*');
		$this->db->from('_na_province');
        $this->db->where('lang', $language);		
...
public function get_province($geo_id,$province_id=0, $province_id=null, $status=null, $order='order_by', $order_type='Asc', $limit_start = 0, $listpage = 2000000000,$lang=''){
		if($lang==null){
			$language = $this->lang->language['lang'];
		}else{
			$language = $lang;
		}
	//	echo'  <hr> <pre>  language=>';print_r($language);echo'</pre>'; Die();
...
function province_count_id($geo_id,$lang=''){
		if($lang==null){ $language = $this->lang->language['lang']; }else{ $language = $lang; 	}
