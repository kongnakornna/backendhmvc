...
public function countfollow_get() {  
 $user_id=$this->input->get('user_id');
 $this->load->model('blog/blogmodel');
 $data=$this->blogmodel->countFollow($user_id);
			$count=count($data);
			 if($count<=0 ||$count==Null){
				$this->response(array('header'=>array(
										'title'=>'countfollow',
...
##### END ######	
/*
public function tagsearch_get($tag_name) {
					 // http://www.trueplookpanya.com/api/center/tagsearch?tagname=fb
      if($tag_name==''){
       $tag_name=@$this->input->get('tagname');
      }
					 
					 $this->load->model('Examination_center_model');
					 $data = $this->Examination_center_model->tagsearch($tag_name);
					 $countall=$countalls['countrow'];
					 $rows=count($data);
					 if($data){
						$this->response(array('header'=>array(
