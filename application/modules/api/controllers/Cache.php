...
$this->load->model('Model_cacheapi', 'model_cache');
}
...
$id=@$this->get('id');
$dmsdata=$this->model_cache->list_all_data_ams_entrance('desc','5');
...
$data_memcachetest_model=$this->model_cache->memcache_manual($deletekey);
...
if($limit==''){$limit='3';}
$data_memcachetest_model=$this->model_cache->get_memcached_result($deletekey,$order_by,$limit);
if($data_memcachetest_model!==''){$this->response(array('header'=>array(
										'title'=>'REST_Controller::HTTP_OK',
										'message'=>' DATA OK',
										'status'=>TRUE,
...
public function memcachetests_get(){
$deletekey=@$this->input->get('deletekey');
$order_by=@$this->input->get('order_by');
$limit=@$this->input->get('limit');
if($order_by==''){$order_by='asc';}
if($limit==''){$limit='3';}
$data_memcachetest_model=$this->model_cache->get_memcachedrs($deletekey,$order_by,$limit);
if($data_memcachetest_model!==''){$this->response(array('header'=>array(
										'title'=>'REST_Controller::HTTP_OK',
										'message'=>' DATA OK',
										'status'=>TRUE,
...
#$data_all1=$this->model_cache->get_where_u_id($u_id);
$data_all=$this->model_cache->list_all_data_ams_entrance('desc',10);
if($data_all!==''){$this->response(array('header'=>array(
										'title'=>'REST_Controller::HTTP_OK',
										'message'=>' DATA OK',
										'status'=>TRUE,
...
$path = $this->config->item('cache_path');
 ...
 if($get_data){
     ...
     $datars=$this->model_cache->get_file_where_u_id($u_id);
     if($datars){
