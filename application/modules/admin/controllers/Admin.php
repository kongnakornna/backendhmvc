 ...
 */
 ...
 public function memberlist(){
   ...
   if(isset($search_form['admin_type_id'])){
    ...
    $langs=$this->lang->line('langs'); 
	$userdata=$this->session->userdata();
	$admin_type_ses=$userdata['admin_type'];
	//Debug($admin_type_ses);Debug($userdata); Die();
   	$key='memberlist_type_'.$admin_type_ses.'_lang_'.$lang.'_'.$admin_type_id;
       if (!$memberlist_cach= $this->cache->get($key)){
             $memberlist_cach= $this->adminfactory->getAdminna(0,Null, $admin_type_id);
            // Save into the cache for 5 minutes
            $this->cache->file->save($key,$memberlist_cach,$timecache);
       }
      $memberlist=$memberlist_cach;
   	/////////////cache////////////
	...
	$admin_type_ses=$userdata['admin_type'];
	//Debug($admin_type_ses);Debug($userdata); Die();
   	$key='member_list_type_'.$admin_type_ses.'_lang_'.$lang.'_'.$admin_type_id.'_keyword_'.$keyword;
       if (!$memberlist_cach= $this->cache->get($key)){
             $memberlist_cach=$this->adminfactory->getAdminna(0, $keyword);
            // Save into the cache for 5 minutes
            $this->cache->file->save($key,$memberlist_cach,$timecache);
       }
      $memberlist=$memberlist_cach;
	...
	$admin_type_ses=$userdata['admin_type'];
	//Debug($admin_type_ses);Debug($userdata); Die();
   	$key='member_list_type_'.$admin_type_ses.'_lang_'.$lang.'_'.$admin_type_id;
       if (!$memberlist_cach= $this->cache->get($key)){
             $memberlist_cach=$this->adminfactory->getAdminna(0, null, $admin_type_id);
            // Save into the cache for 5 minutes
            $this->cache->file->save($key,$memberlist_cach,$timecache);
   ...
   //Debug($getAdminType);
			//Debug($admin_type_list);
			  $data=array("admin_menu" => $this->menufactory->getMenu(),
            				 "getAdminType" => $getAdminType,
            					"memberlist1" => $this->adminfactory->getAdmin(),
            					"memberlist" => $memberlist,
                 "admin_type_list" => $admin_type_list,
