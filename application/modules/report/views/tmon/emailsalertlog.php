...
<?php 
		...
		$i=0;
		$maxcat = count($emailsalertlog_list);
		$lang=$language['lang']; 
?>
<style type="text/css">
...
<?php
	...
	echo form_open('emailalertlog', $attributes);
?>
...
<div>
	<table>-->
												<thead>
														<tr><th width="39">ID</th>
														<th width="51"><?php echo $language['no'] ?>.</th>
														<th width="211"><?php echo $language['sensor'] ?></th>
														<th width="137"><?php echo $language['sensorvalue'] ?></th>
														<th width="322" class="hidden-480"><?php echo $language['alarmlogreport'] ?> </th>
														<th width="277"> <?php echo $language['email'] ?> </th>
														<th width="231"> <i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i><?php echo $language['datetime'] ?> </th>
													</tr>
												</thead>
				<tbody>
<?php
				...
				$i=1;
				//$i=0;
				if($emailsalertlog_list)
				for($key=0;$key<$maxcat;$key++){
							$mail_id = $emailsalertlog_list[$key]['mail_id'];
							$sensor_name = $emailsalertlog_list[$key]['sensor_name'];
							$value = $emailsalertlog_list[$key]['value'];
							$status = $emailsalertlog_list[$key]['status'];
							$to = $emailsalertlog_list[$key]['to'];
							$date = $emailsalertlog_list[$key]['date'];
...
?>
		...
		<tr>
						...
						<td><a href="#"><?=$mail_id?></a></td>
						<td><?php echo $i;?></td>
						<td><a href="#"><?=$emailsalertlog_list[$key]['sensor_name']?></a></td>
						<td><a href="#"><?=$emailsalertlog_list[$key]['value']?></a></td>
						<td><a href="#"><?=$emailsalertlog_list[$key]['status']?></a></td>
						<td>
						<?php if($to<>''){echo $to;}?> 
						</td>
						<td> <?php $create_date=$emailsalertlog_list[$key]['date'];
...
<?php
		if($emailsalertlog_list)		
				for($key=0;$key<$maxcat;$key++){
						...
						$mail_id = $emailsalertlog_list[$key]['mail_id'];
?>
		$('#status<?=$mail_id?>').on('click', function() {
				var id = $(this).attr('id');
		    	var n = id.length;
			    var maxstr = n-6;
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url('emailsalertlog/status/'.$mail_id)?>",
					cache: false,
		...
		$('#bootbox-confirm<?=$mail_id?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('admin_menu/delete/'.$mail_id)?>';
						}
					});
		});

		$('#bx-confirm<?=$mail_id?>').on('click', function() {
					bootbox.confirm("<?php echo $language['are you sure to delete']?>", function(result) {
						if(result) {
								window.location='<?php echo base_url('admin_menu/delete/'.$mail_id)?>';
						}
