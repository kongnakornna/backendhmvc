...
$maxdata=$maxdata[0]['maxdata'];
$maxdatana=$maxdata+40;
#echo '$maxdata='.$maxdata; Die();
 					...
 					<div class="row">
						<div class="col-md-12">
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									<i class="fa fa-external-link-square"></i>
									Real Time Chart : <?php echo $this->lang->line('temperature');?> (Max <?php echo $maxdata;?>)
									<div class="panel-tools">
...
?>
        var sensor<?#tooltipphp echo $keyno;?> = <?php
        	$url=base_url();
			$json_string=$url.'/api/jsonfile/HW'.$hardware_id.'_sensor'.$keyno.'_'.$sensor_type.'.json';   
			$jsondata=file_get_contents($json_string);
			$data_ret=json_decode($jsondata,true);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        var updateInterval = <?php echo $maxdata;?>;
        ...
        var plot = $.plot("#placeholder4", [getRandomData()], {
            ...
            /////////////// Data From Database/////////////
            yaxis: {
                min: -<?php echo $maxdata;?>,
                max: <?php echo $maxdatana;?>,
            },
            xaxis: {
                show: false
            }
...
var Main = function() {
	...
	var runModuleTools = function() {
		...
		$('.panel-tools .panel-refresh').on('click', function(e) {
			...
			el.block({
				...
				message: '<img src="<?php echo base_url('theme');?>/assets/images/loading.gif" /> Just a moment...',
				css: {
					border: 'none',
					color: '#333',
					background: 'none'
	...
	var setColorScheme = function() {
		$('.icons-color a').on('click', function() {
			...
			$('#skin_color').attr("href", "<?php echo base_url('theme');?>/assets/css/theme_" + $(this).attr('id') + ".css");
	...
	var runActivateLess = function() {
		...
		if($('#skin_color').attr("rel") == "stylesheet") {
			$('#skin_color').attr("rel", "<?php echo base_url('theme');?>/stylesheet/less").attr("href", "<?php echo base_url('theme');?>/assets/less/styles.less");
	...
	var runSaveSetting = function() {
		$('.save_style').on('click', function() {
			...
			el.block({
				...
				message: '<img src="<?php echo base_url('theme');?>/images/loading.gif" /> Just a moment...',
				css: {
					border: 'none',
					color: '#333',
					background: 'none'
