<?php
class Cachetool_model extends CI_Model{
...
public function deletekey($uri_string,$filecachename){
        $cache_path=FCPATH.'file/cache/';
        $cache_path=$cache_path.$filecachename;
       if(file_exists($cache_path)) { 
          $messagefile='Have file '.$cache_path;
      }else{
          $messagefile='Not Have file '.$cache_path;
      }
       $unlinkstatus=@unlink($cache_path);
       if($unlinkstatus==1){
         $message='Delete Cache key '.$filecachename.' Complate New Rewrite data';
         $status='1';
       }else{
           $message='Not have Cache key '.$filecachename;
           $status='0';
       }
         $cache_path_delete=array("messagefile" => $messagefile,
                                   "path"=> $cache_path,
                                   "cache_path"=> $cache_path,
                                   "unlink_status" => $unlinkstatus,
                                   "message" => $message,
...
public function write_cache_md5(){
        $uri=$CI->config->item('base_url').$CI->uri->uri_string();
        // Chalda here: cache in subdirectory
        $md5=md5($uri);
        $cache_path=FCPATH.'file/cache/'.substr($md5,0,1).'/';
...
public function display_cache_md5($cache_path){
...
public function clear_all_cache_md(){
    $CI =& get_instance();
    $path = $CI->config->item('cache_path');
    $cache_path = ($path == '') ? APPPATH.'cache/' : $path;
    $handle = opendir($cache_path);
