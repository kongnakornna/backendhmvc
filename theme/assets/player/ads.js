...
Ads.prototype.requestAds = function(adTagUrl) {
  ...
  this.application_.statusApplication = "ad";
  this.application_.resize();
};

Ads.prototype.requestAdsOverlay = function(adTagUrl) {
  this.adstype = "overlay";
  var adsRequest = new google.ima.AdsRequest();
  adsRequest.adTagUrl = adTagUrl;
  adsRequest.linearAdSlotWidth = this.videoPlayer_.width;
  adsRequest.linearAdSlotHeight = this.videoPlayer_.height;
...
Ads.prototype.skip = function() {
   /*  this.adsManager_.skip();
   */
  if (this.adsManager_) {
...
Ads.prototype.onAdEvent_ = function(adEvent) {
  ...
  for (i = 0; i < frames.length; ++i){
      if(frames[i].src.indexOf("imasdk") > 0){
          frameAds = frames[i];
          var innerDoc = (frameAds.contentDocument) ? frameAds.contentDocument : frameAds.contentWindow.document;

          //var innerDoc = frameAds.contentDocument || frameAds.contentWindow.document;
          //var arrayAds = innerDoc.getElementsByClassName('videoAdUiPreSkipContainer')[0];
          //var frames = document.getElementsByTagName("body");
          console.log(frameAds.getElementsByTagName("body").length);
          //console.log("frame : "+frames[i].src);
      }
  ...
  */
  ...
  if(adEvent.type == google.ima.AdEvent.Type.STARTED){
      if(this.adstype == "preroll"){
        var myAd = adEvent.getAd();
        if(!myAd.isSkippable()){  
            this.application_.countdownAd(this.application_);
        }
      }
  }
