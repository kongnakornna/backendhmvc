...
<div class="filter">
  ...
  <input type="hidden" name="pid" value="<?=$this->input->get('pid')?>">
  ...
  <button type="submit" class="btn btn-default"><span>Filter</span></button>
  <a href="/V3/sitemin/cmsblog/cms-list-children?pid=<?=$this->input->get('pid')?>"><span  class="btn btn-default">Clear</span></a>
  <a href="/V3/sitemin/cmsblog/cms-detail?parent_idx=<?=$this->input->get('pid')?>"><span class="btn btn-default">ADD</span></a>
  </div>
...
<tbody>
  <?php if($lists) { ?>
    ...
    <tr>
      <td><?=$row['idx']?></td>
      <td><?=$row['parent_idx'] ? $row['parent_idx'] : '-'?></td>
      <td>
          <?php
           //var_dump($row);
          /*
      ...
      <td>
        <?=$row['child_order'] > 0 ? $row['child_order']. " | " : '-'?> &nbsp;
        <?php if($lists[$index] != end($lists) && $lists[$index] != reset($lists) && isset($lists[$index + 1]) && $lists[$index+1]['child_order'] > $lists[$index]['child_order'] ) { ?>
          <a href="<?=site_url(ADMIN_PATH."/cmsblog/swap-child-order/".$row['parent_idx']."/".$row['idx']."/".$row['child_order']."/".$lists[$index+1]['idx']."/".$lists[$index+1]['child_order'])?>"><i class="btn btn-danger btn-xs glyphicon glyphicon-arrow-down" ></i></a>
        <?php } ?>
        <?php if($lists[$index]['child_order'] > 1 && isset($lists[$index - 1]) && $lists[$index-1]['child_order'] < $lists[$index]['child_order'] )  { ?>
          <a href="<?=site_url(ADMIN_PATH."/cmsblog/swap-child-order/".$row['parent_idx']."/".$row['idx']."/".$row['child_order']."/".$lists[$index-1]['idx']."/".$lists[$index-1]['child_order'])?>"><i class="btn btn-warning btn-xs glyphicon glyphicon-arrow-up" ></i></a>
        <?php } ?>
      </td>
      <td nowrap>
