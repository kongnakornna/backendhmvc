...
CKEDITOR.dialog.add( 'image2', function( editor ) {
    ...
    responsiveButtonId = CKEDITOR.tools.getNextId(),

		lang = editor.lang.image2,
		commonLang = editor.lang.common,

		lockResetStyle = 'margin-top:18px;width:65px;height:20px;',
		lockResetHtml = new CKEDITOR.template(
			'<div>' +
				'<a href="javascript:void(0)" tabindex="-1" title="' + lang.lockRatio + '" class="cke_btn_locked" id="{lockButtonId}" style="float:left;" role="checkbox">' +
					'<span class="cke_icon"></span>' +
					'<span class="cke_label">' + lang.lockRatio + '</span>' +
				'</a>' +

				'<a href="javascript:void(0)" tabindex="-1" title="' + lang.resetSize + '" class="cke_btn_reset" id="{resetButtonId}" style="float:left; margin-left:5px;" role="button">' +
					'<span class="cke_label">' + lang.resetSize + '</span>' +
				'</a>' +
        
        '<a href="javascript:void(0)" tabindex="-1" title="' + lang.responsiveSize + '" class="cke_button_icon cke_button__maximize_icon" style="float:left; margin-left:5px;" id="{responsiveButtonId}" role="button">' +
					'<span class="cke_label">' + lang.resetSize + '</span>' +
				'</a>' +
			'</div>' ).output( {
				lockButtonId: lockButtonId,
				resetButtonId: resetButtonId,
        responsiveButtonId: responsiveButtonId
        } ),
		...
		lockButton, resetButton, responsiveButton,widthField, heightField, captionTextField, captionLinkField, hasCaptionField,
    ...
    responsiveButton = doc.getById( responsiveButtonId );
    ...
    // Activate the reset size button.
		if ( responsiveButton ) {
			// Consider that there's an additional focusable field
			// in the dialog when the "browse" button is visible.
			dialog.addFocusable( responsiveButton, 6 + hasFileBrowser );

			// Fills width and height fields with the original dimensions of the
			// image (stored in widget#data since widget#init).
			responsiveButton.on( 'click', function( evt ) {
				// If there's a new image loaded, reset button should revert
				// cached dimensions of pre-loaded DOM element.
				if ( srcChanged ) {
          widthField.setValue( 0 );
			...
			setupMouseClasses( responsiveButton );
		}
	}
  ...
  function getUrlOnly(url){
    return url.replace(/#[^#]*$/, "").replace(/\?[^\?]*$/, "").replace(/^https:/, "http:");
  }
	...
	return {
		...
		contents: [
					...
					// },
          {
						id: 'captionText',
						type: 'text',
						label: lang.captionPlaceholderText,
						requiredContent: features.caption.requiredContent,
            onLoad: function() {
              captionTextField = this;
            },
						setup: function( widget ) {
            ...
            validate : function( widget ) {
              if(config.requireHasCaption){
                if ( !this.getValue() ) {
                  alert( lang.captionMissing );
                  return false;
                }else{
                  return true;
                }
					...
					},
          {
						id: 'captionLink',
						type: 'text',
						label: lang.captionPlaceholderLink,
						requiredContent: features.caption.requiredContent,
            onLoad: function() {
              captionLinkField = this;
            },
						setup: function( widget ) {
							this.setValue( widget.data.captionLink );
						},
						commit: function( widget ) {
							widget.setData( 'captionLink', this.getValue() );
						}
					},
				]
			},
